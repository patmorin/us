...
\begin{lem}\label{three-tree-layers}
    ...
    \begin{compactenum}[(i)]
        ...
        \item For each $i\in\{1,\ldots,m\}$, if $u_0,\ldots,u_r$ is a path in $G$ with $\{u_0,u_r\}\subseteq L_i$ and $\{u_1,\ldots,u_{r-1}\}\subseteq \bigcup_{j=i+1}^m L_i$ then $u_0u_r\in E(G)$.
    \end{compactenum}
\end{lem}
...
\begin{lem}\label{three-tree-technical}
    ...
    Let $G$ be a planar 3-tree with construction order $\mathbb{R}:=v_1,\ldots,v_n$ and let $\mathcal{L}:=L_0,\ldots,L_m$ be a BFS layering of $G$ with $L_0:=\{v_1,\ldots,v_t\}$, let $X\subset L_i$ and let $G_X$ be the graph containing precisely the connected components of $H[X\cup\bigcup_{j=i+1}^m L_j]$ that contain elements of $X$.  If
    \begin{equation}
        |G_X| \le \frac{(\log k)^k}{(\log(c+\log c/\log\log c))^{c+\log c/\log\log c}}
        \enspace , \label{input-size}
    \end{equation}
    then there exists an $\ell$-ranking $\varphi:V(G_X)\to\{1,\ldots,ak\}$ of $G$ such that $\varphi(v) \le a(k-c-1)-g$ for each $v\in X$.
\end{lem}

\begin{proof}
    The proof is by induction on $m-i$.  For each vertex $v\in V(H[X])$, let $G_v$ be the graph containing exactly the components of $H[\bigcup{j=i+2}^m]$ having at least one vertex $w$ adjacent to an element of $N_G(v)\cap L_{i+1}$.
    ...
    By \cref{three-tree-layers}(i), $G[X]\subseteq G[L_i]$ is an outerplanar graph. Let $s:=\log c/\log\log c$. By \cref{size-claim,input-size}, $\sum_{v\in X}(\log k)^k/(\log\gamma(v))^{\gamma(v)} \le 3(\log k)^k/(\log(c+s))^{c+s}$.  Therefore, the graph $G[X]$ and the weight function $\gamma$ satisfy the requirements for \cref{outerplanar-lemma} with $\alpha =3$. Applying \cref{outerplanar-lemma} with $g=\ell+1$, we obtain an $\ell$-ranking  $\varphi:X\to\{1,\ldots,a(k-c-1)\}$ of $G[X]$ such that $\varphi(v)>a(k-\gamma(v)-1)-\ell-1$ for each $v\in X$.

    In the base case, $i=m$, we are done; $\varphi$ is a complete colouring of $G_X$ that satisfies the requirements of the lemma.
    ...
    \begin{compactenum}
        \item $|H|\le \beta|G_v|/\gamma(v)$; or
        \item $V(H)\cap Y=\emptyset$.
    \end{compactenum}
    ...
    \[  \frac{\beta|G_v|}{\gamma(v)} \le \frac{(\log k)^k}{\log (\gamma(v)+s(v))^{\gamma(v)+s(v)}}
    \]
    where $s(v):=\log\gamma(v)/\log\log\gamma(v)$. The constant $\beta$ is universal, so $|S_v|\in O(\gamma(v))$.

    Let $S:=\bigcup_{v\in X} S_v'$ and let $\{S_v: v\in X\}$ be partition of $S-Y_{\ge 2}$ such that $S_v\subseteq S_v'$ for each $v\in X$.  We colour the vertices in $S$ as follows:  For each $v\in X$, we colour $S_v$ with $|S_v|$ distinct colours from the set $\{\lfloor a(k-\gamma(v)-1)\rfloor+1,\ldots,ak\}$.  Since $|S_v|\le |S_v|\in O(\gamma(v))$, this is always possible, provided that $a$ is a sufficiently large constant.

    % Now, consider the graph $G_v-S_v$.
    Finally, let $Q:=G_Y-S-Y_{\ge 2}$.  We apply induction on each component $I$ of $Q$ to obtain an $\ell$-ranking $\varphi:V(I)\to\{1,\ldots,ak\}$ of $I$ such that, for each $w\in I[Y]$, $\varphi(w) \le a(k-\gamma(I)-1)$, where $\gamma(I)$ is the solution to the equation $(\log k)^k/(\log\gamma(I))^{\gamma} = |I|$.  Notice that there is a set $C_I\subset L_i$ of size 3 such that $\gamma(I)\ge \max\{\gamma(v):v\in C_I\cap X\}$. Indeed, for each $v\in C_I\cap X$, $I$ is contained in a component of $G_v-S_v$ and, as argued above, each such component has size at most $(\log k)^k/\log (\gamma(v)+s(v))^{\gamma(v)+s(v)}$.
    ...
    All that remains is verify that this produces an $\ell$-ranking $G_X$.  Specifically, we will show that for any path $u_0,\ldots,u_r$ in $G_X$ of length $r\le\ell$, at least one of the following conditions holds:
    \begin{compactenum}[(i)]
        \item $\varphi(u_0)\neq\varphi(u_r)$, $\delta(u_0)\neq\delta(u_r)$, $\tau(u_0)\neq\tau(u_r)$; or
        \item $\varphi(u_0) < \varphi(u_j)$ for some $j\in\{1,\ldots,r-1\}$
    \end{compactenum}
    Assume, for the sake of contradiction that $u_0,\ldots,u_r$ is a minimum-length path that does not satisfy Condition~(i) or (ii).
    ...
    \begin{compactenum}[(P1)]
        \item $u_0,\ldots,u_r$ is an induced path since, otherwise, the shorter path from $u_0$ to $u_r$ contained in $u_0,\ldots,u_r$ does not satisfy Conditions~(i) or (ii), contradicting the minimality of $u_0,\ldots,u_r$.

        \item $\{u_0,u_r\}\subseteq L_p$ for some $p\in\{i,\ldots,m\}$ since, otherwise $\delta(u_0)\neq\delta(u_r)$.

        \item $\{u_1,\ldots,u_{r-1}\}\subseteq V(G_X[\bigcup_{j=i}^p L_p])$ since, otherwise, \cref{three-tree-layers}(iii) implies $u_0,\ldots,u_r$ is not an induced path.
    ...
    We say that $u_0,\ldots,u_r$ is $\tau$-uniform if $\tau(u_0)=\tau(u_1)=\cdots=\tau(u_r)$.

    Since $\tau(u_0)=\tau(u_r)$, there are four major cases to distinguish:
    \begin{inparaenum}[(1)]
        \item $u_0,u_r\in X$;
        \item $u_0,u_r\in S$;
        \item $u_0,u_r\in Y_{\ge 2}$; or
        \item $u_0,u_r\in V(Q)$.
    \end{inparaenum}
    We now consider each case:
    \begin{compactenum}[(1)]
        \item $u_0,u_r\in X$. Then, by (P3), $u_0,\ldots,u_r$ is $\tau$-uniform. This is a contradiction since $\varphi$ is an $\ell$-ranking of $G_X[X]$ by the application of \cref{outerplanar-lemma}.

        \item $u_0,u_r\in S$. There are several subcases to consider.
        \begin{compactenum}
            \item $u_0,u_r\in S_v$ for some $v\in X$.  In this case $\varphi(u_0)\neq\varphi(u_r)$.

            \item $u_0\in S_v$, $u_r\in S_w$ for some disinct $u,w\in X$. This is the hard part.!!!!  For $\ell=2$, we can prove that $\psi(u_0)\neq\psi(u_r)$ \emph{or} that one of $u_0$ or $u_r$ is in
        \end{compactenum}

        \item $u_0,u_r\in Y_{\ge 2}$.

        In this case, $u_0,\ldots,u_r$ is not $\tau$-uniform since, otherwise, $u_0,\ldots,u_r$ is contained in $G[Y_{\ge 2}]$, a contradiction since $\varphi$ is a $\ell$-ranking of $G[Y_{\ge 2}]$.  Therefore there exists some
        $j\in\{1,\ldots,r-1\}$ such that $\tau(u_0)=\cdots=\tau(u_{j-1})\neq \tau(u_j)$.  There are three cases to consider:
        \begin{compactenum}
            \item $u_j\in X$: then $\varphi(u_0)<\varphi(u_j)$
        ...
        \item $u_0,u_r\in V(Q)$.
        \begin{compactenum}
            \item $u_1,\ldots,u_r$ contains a vertex of $L_{p-1}$.
            \item $u_1,\ldots,u_r$
        \end{compactenum}
