...
% The set of subgraphs of diameter at most $\ell$ includes all connected subgraphs with at most $\ell+1$ vertices.
...
\[
    \log^{(i)} x :=
      ...
      \end{cases}
\]
Define the \emph{$\tau$ower} function $\tau:\N\setminus\{0\}\to\N$ by
\[
  \tau(i) :=
    \begin{cases}
        1 & \text{for $i=0$} \\
...
For any integer $i$, any real $k>\tau(i-1)$, and any real $n\in[1,(\log^{(i)} k)^k]$, we define $\gamma_{i,k}(n)$ to be the solution $x\in[\tau(i),k]$ to the equation
\begin{equation}
...
\begin{lem}\label{path-induced}
    ...
    \begin{compactenum}[(Z1)]
        ...
        \item $|U|\in O((\ell+1)^t)$; and \label{size-u}
...
\begin{proof}
    ...
    For each $i\in\{1,\ldots,p+1\}$, let $P_i':=y_{i-1},\ldots,y_i$, let $G'_i:=G'[\bigcup_{j=y_{i-1}}^{y_i}B'_j]$ and let $\mathcal{P}'_i:=(B'_x:x\in V(P'_i)\}$.
    ...
    In particular, $U$ contains $B_{y_0}=B_1$ and $B_{y_{p+1}}=B_m$, so $U$ satisfies (Z\ref{values-u}). The size of $U$ satisfies the recurrence $f(0)\le 2$ and $f(t)\le \ell+1+(\ell+1)\cdot f(t-1)$, which resolves to $f(t)\le ((3\ell+1)(\ell+1)^t-x-1)/\ell \in O((\ell+1)^t)$, so $U$ satisfies (Z\ref{size-u}).  All that remains is to show that $U$ satisfies (Z\ref{induced-u}).  Consider some induced path $w_0,\ldots,w_q$ in $G$ of length at most $\ell$ with $\{w_0,w_q\}\subseteq U$.  We want to show that $\{w_1,\ldots,w_{q-1}\}\subseteq U$.
    ...
    \begin{compactenum}
       ...
       \item $w_a = u_j$ for some $j\in\{0,\ldots,p\}$ and $w_b\in V(G_{j+1}')$.  In this case, $w_a=u_j\in B_{k}$ for each $k\in\{y_j,\ldots,y_{j+1}\}$ and $w_b\in B_{k}$ for at least one $k\in\{y_j,\ldots,y_{j+1}\}$.  By edge maximality, $w_aw_b\in E(G)$ and $b=a+1$ so there is nothing to prove.  \qedhere
       %
       %
...
We proceed by induction on $n$. In the base case, $n=1$, so $c=k$.  We set $\varphi(r):=\floor{a(k-c)}+1=1$ and we are done.\todo{Change this everywhere} Now suppose $n\ge 2$.  For each $v\in L_{\ell+1}$, apply the inductive hypothesis on the subtree $T_v$ to obtain an $\ell$-ranking $\varphi$ of the forest $F:=T[\bigcup_{i=\ell+1}^h L_i]$.
...
\todo[inline]{
TODO: Either put the $\beta$ into \cref{total-weight} or find a way around it.

TODO: Get rid of the ${}-1$'s everywhere
}
...
\begin{proof}
    Without loss of generality, we may assume $H$ is edge-maximal with respect to some $r$-rooted $t$-simple tree decomposition $\mathcal{T}:=(B_x:x\in V(T))$.  Let $L_0,\ldots,L_h$ be a BFS layering of $H$ with $L_0:=B_r$.\todo{Do I use $:=$ everywhere?}  Note that $h\le\diam(H)\le d$.
