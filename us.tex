...
\begin{thm}\label{t-trees}
    [BROKEN: The upper bound doens't work right now.] For every fixed $t$ and every $n$-vertex $t$-tree, $\uqs(G) \in O(\log n/\log^{(t+1)}(n)$ and this is asymptotically optimal: for infinitely many values of $n$, there exists an $n$-vertex $t$-tree $T$ with $\uqs(T)\in\Omega(\log n/\log^{(t+1)} n)$.
\end{thm}
...
[I've moved the original section on colouring $2$-trees to \cref{two-trees-appendix}.  This section is currently in the process of changing $2$ to $t$.]

[BROKEN: Right now, this section is broken.  I'm unable to prove the slack lemma for weighted $(t-1)$-trees, for $t\ge 3$.]
...
\begin{compactenum}
    ...
    \item Each component of $F-S$ can be us-coloured using $O(k-c)$ small colours $\{1,\ldots,a(k-c-1)\}$. [BROKEN: I'm unable to prove this for $t>2$.]
\end{compactenum}
...
\begin{proof}
    ...
    \[
        n_v > \frac{(\log^{(t-1)} k)^k}{\left(\log^{(t-1)}\left(c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}\right)\right)^{c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}}} \enspace ,
    \]
    and $v$ is \emph{light} otherwise.  For a heavy node $v$,
    \begin{align*}
        (\log^{(t-1)} k)^k/n_v & < \left(\log^{(t-1)}\left(c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}\right)\right)^{c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}} \\
        & = \left(\log^{(t-1)}(c+s)\right)^{\left(c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}\right)
            \left(\tfrac{\log^{t}(c+s+\log^{(t-1)}(c+s)/\log^{(t)}(c+s))}{\log^{(t)}(c+s)}\right)} \\
        & < (\log^{(t-1)}(c+s))^{\left(c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}\right)
            \left(1 + \tfrac{(\log e)^t(\log^{(t-1)}(c+s))/\log^{(t)}(c+s)}{\prod_{j=0}^{t}\log^{(j)}(c+s)}\right)} \\
        & = (\log^{(t-1)}(c+s))^{\left(c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)}\right)
            \left(1 + \tfrac{(\log e)^t}{(c+s)\left(\prod_{j=1}^{t-2}\log^{(j)}(c+s)\right)(\log^{(t)}(c+s))^2}\right)} \\
        & = (\log^{(t-1)}(c+s))^{c+s+\tfrac{\log^{(t-1)}(c+s)}{\log^{(t)}(c+s)} +
            \tfrac{(\log e)^t}{(\log^{(t)}(c+s))^2} + \tfrac{(\log e)^t\log^{(t-1)}(c+s)}{(c+s)\left(\prod_{j=1}^{t-2}\log^{(j)}(c+s)\right)(\log^{(t)}(c+s))^3}}  \\
        & = (\log(c+s))^{c+s}\cdot (\log^{(t-2)}(c+s))\cdot (1+o_c(1)) XXXXXXXXXXXXXXXXXXXXXX \enspace .
        \numberthis \label{sizer}
    \end{align*}

    Let $T'$ be the subtree of $T$ induced by the set of heavy nodes.  Since no leaf of $T'$ is a $T$-descendant of any other leaf, it follows from \cref{total-weight-i,sizer} that the number of leaves of $T'$ is at most
    $(\log^{(t-2)}(c+s)\cdot (\log e)^t\cdot (1+o_c(1))\in O(\log^{(t-2)}(c+s))$.

    Let $H':=H[\bigcup_{x\in V(T')} B_x]$ and notice that $\mathcal{T}':=(B_x:x\in V(T'))$ is a width-$(t-1)$ tree decomposition of $H'$. The tree $T'$ has $O(\log^{(t-2)} c)$ leaves and therefore has at most $O(\log^{(t-2)} c)$ branching nodes. By \cref{skeleton-size}, the $\mathcal{T}'$-skeleton $\hat{H'}$ is a $t$-tree and $|\hat{H}'|\in O(\log^{(t-2)} c)$.  By applying \weirdref{t-trees}{a} for $t-1$, we conclude that
    \[  \uqs(\hat{H}')\in O\left(\frac{\log|\hat{H}'|}{\log^{(t-1)}|\hat{H}'|}\right) \subseteq O\left(\frac{\log^{(t-1)} c}{\log^{(t)} c}\right) = O(s) \enspace .
    \]
...
\begin{proof}
    ...
    \[
        n_v> \frac{\left(\log^{(t-1)} k\right)^k}{\log^{(t-1)}\left(c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}\right)^{c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}}} \enspace ,
    \]
    otherwise $v$ is \emph{light}.  For a heavy node $v$,
    \begin{align*}
        \left(\log^{(t-1)} k\right)^k/n_v
        & < \left(\log^{(t-1)}\left(c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}\right)\right)^{c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}} \\
        & = \left(\log^{(t-1)}c\right)^{\left(c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}\right)
        \left(\tfrac{\log^{(t)}\left(c+\log^{(t-1)} c/\log^{(t)} c\right)}{\log^{(t)} c}\right)} \\
        & = \left(\log^{(t-1)}c\right)^{\left(c+\tfrac{\log^{(t-1)} c}{\log^{(t)} c}\right)
        \left(
        1 + \tfrac{(\log e)^t\log^{(t-1)} c/\log^{(t)} c}{\prod_{j=0}^{t}\log^{(j)}c}
        \right)} \\
    \end{align*}
...
\section{The Upper Bound for 2-Trees}

\label{two-trees-appendix}
[This is the original proof for 2-trees.]

Before working directly with 2-trees we will work with node-weighted trees. The following lemma gives a condition under which we can colour a weighted tree with significantly fewer than $k$ colours.

% explains how to proceed if the ratio between the total weight and maximum individual weight is sufficiently small.  When we have this slack, we can colour the tree with significantly less than $k$ colours.

\begin{lem}\label{tree-slack}
Let $t,k\in\N$, $c,s\in\R$ be such that $2< c\le k-1$, let $T$ be a non-empty tree with root $r$,  let $\gamma:V(T)\to\{z\in\R: z\ge 2\}$, and define
\[
    n_r:=\sum_{v\in V(T)} \frac{(\log k)^k}{(\log \gamma(v))^{\gamma(v)}} \enspace .
\]
There exists constants $a,b>0$ such that,
if
\begin{equation}
     n_r \le \frac{(\log k)^k}{\left(\log (c+s))\right)^{c+s}} \enspace , \label{total-weight-i}
\end{equation}
where
\begin{equation}
    s := \frac{\log c}{\log\log c} \enspace ,
\end{equation}
% and
% \begin{equation}
%     \phi\in\left\{\lfloor a(k-c-s-1)\rfloor+1,\ldots,a(k-c-1)\right\} \label{phi-bound}
% \end{equation}
then $T$ has a humped us-colouring $\varphi$ such that
\begin{compactenum}[(P1)]
    \item $\varphi(r)=\lfloor a(k-c-1)\rfloor$; and
    \item for each $v\in V(T)$, $a\cdot(k-\gamma(v)-1) < \varphi(v))\le a(k-c)$.
\end{compactenum}
\end{lem}
...
\begin{proof}
    ...
    \[ \frac{(\log k)^k}{(\log\gamma(r))^{\gamma(r)}}\le n_r \le \frac{(\log k)^k}{(\log (c+s))^{c+s}}
    \]
    so $\gamma(r) \ge c+s \ge c+1$ for $s\ge \log c/\log\log c\ge 1$, which is satisfied for all $c>2$. Therefore, \cref{phi-bound} ensures that (P2) is satisfied.

    Now assume $|V(T)|\ge 2$.  For each node $v\in V(T)$, let $n_v:=\sum_{w\in T_v} (\log k)^k/(\log\gamma(w))^{\gamma(w)}$, where $T_v$ denotes the subtree of $T$ induced by $v$ and all its $T$-descendants. Note that, for $v=r$, this corresponds with the definition of $n_r$ in the statement of the lemma.  We say that node $v$ of $T$ is \emph{heavy} if
    \[
        n_v > \frac{(\log k)^k}{\left(\log\left(c+s+\tfrac{\log(c+s)}{\log\log(c+s)}\right)\right)^{c+s+\tfrac{\log(c+s)}{\log\log(c+s)}}} \enspace ,
    \]
    and $v$ is \emph{light} otherwise.  For a heavy node $v$,
    \begin{align*}
        (\log k)^k/n_v & < \left(\log\left(c+s+\tfrac{\log(c+s)}{\log\log(c+s)}\right)\right)^{c+s+\tfrac{\log(c+s)}{\log\log(c+s)}} \\
        & = (\log(c+s))^{\left(c+s+\tfrac{\log(c+s)}{\log\log(c+s)}\right)
            \left(\tfrac{\log\log(c+s+\log(c+s)/\log\log(c+s))}{\log\log(c+s)}\right)} \\
        & < (\log(c+s))^{\left(c+s+\tfrac{\log(c+s)}{\log\log(c+s)}\right)
            \left(1 + \tfrac{(\log^2 e)(\log(c+s))/\log\log(c+s)}{(c+s)\log(c+s)}\right)} \\
        & = (\log(c+s))^{\left(c+s+\tfrac{\log(c+s)}{\log\log(c+s)}\right)
            \left(1 + \tfrac{\log^2 e}{(c+s)\log\log(c+s)}\right)} \\
        & = (\log(c+s))^{c+s+\tfrac{\log(c+s)}{\log\log(c+s)} +
            \tfrac{\log^2 e}{\log\log(c+s)} + \tfrac{(\log^2 e)\log(c+s)}{(c+s)(\log\log(c+s))^2}}  \\
        & = (\log(c+s))^{c+s}\cdot c\cdot (\log^2 e)\cdot (1+o_c(1)) \enspace .
        \numberthis \label{sizer}
    \end{align*}

    Let $T'$ be the subtree of $T$ induced by the set of heavy nodes.  Since no leaf of $T'$ is a $T$-descendant of any other leaf, it follows from \cref{total-weight-i,sizer} that the number of leaves of $T'$ is at most
    $c\cdot (\log^2 e)\cdot (1+o(1))\in O(c)$.

    Therefore the skeleton $\hat{T}'$ of $T'$ has $O(c)$ nodes so, by \cref{trees}, $\uqs(\hat{T}')\in O(\log (c)/\log\log (c))$. Therefore, by \cref{skeleton-colour}, $T'$ has a us-colouring $\varphi:V(T')\to \{a(k-c-1)-r,\ldots,a(k-c-1)\}$ for some $r\in O(\log c/\log\log c)$ in which $\varphi(r)=a(k-c-1)$.

    Since $r\in O(\log (c)/\log\log (c))$ and $s=\log c/\log\log c$, there exists a constant $a$, such that $a(k-c-1)-r > a(k-c-s-1)$, so each node $v$ in $T'$ receives a colour larger than $a(k-c-s-1)$.  Recall that, for any node $v\in V(T)$, $\gamma(v)\ge c+s$, so $\varphi(v)>a(k-c-s-1)\ge a(k-\gamma(v)-1$  Therefore, the colouring $\varphi$ of $T'$, which is partial colouring of $T$ satisifies requirements (P1) and (P2).
   ...
   \[
       n_{r_1} \le \frac{(\log k)^k}{\left(\log\left(c+s+\tfrac{\log (c+s)}{\log\log (c+s)}\right)\right)^{c+s+\tfrac{\log (c+s)}{\log\log(c+s)}}} \enspace .
   \]
   Therefore, we can apply induction on each tree $T_i$ with the value $c'=c+s$.
   When we do this we obtain a us-colouring of each tree $T_i$ in which $\varphi(r_i) = a(k-c-s-1) < a(k-c-1)-r \le \varphi(v)$ for every node $v$ in $V(T')$.  This ensures that the union of all colourings obtained this way is indeed a us-colouring of $T$.
\end{proof}

The next lemma tells us what to do when we don't have the slackness requirement of \cref{tree-slack}.   In this case we use the top $O(c)$ colours (which we can only use once) to create the slack needed to apply \cref{tree-slack}.


\begin{lem}\label{tree-ii}
Let $t,k\in\N$ and $c\in\R$ with $2< c\le k-1$, let $T$ be a non-empty forest with root $r$, let $\gamma:V(T)\to\{z\in\Z:z\ge 2\}$.  Then there exists a constant $a>0$ such that,
if
\begin{equation}
   n_r:=\sum_{v\in V(T)} \frac{(\log k)^k}{(\log \gamma(v))^{\gamma(v)}} \le \frac{(\log k)^k}{(\log c)^c} \enspace . \label{total-weight-ii}
\end{equation}
% and
% \begin{equation}
...
\begin{compactenum}[(P1)]
    ...
    \item for each $v\in V(T)$, $\varphi(v)> a\cdot(k-\gamma(v))$.
\end{compactenum}
\end{lem}

\begin{proof}
    The proof is by induction on $|V(T)|$. In the base case, $|V(T)|=1$, we set $\varphi(r):=ak$.  This certainly satisifies (P1) and (P2).
    % Furthermore, \cref{total-weight-ii} implies that $\gamma(r)\ge c$.  Therefore \cref{phi-bound-ii} ensures that (P3) is satisfied.

    Now we may assume that $|V(T)|\ge 2$.  For each node $v$ of $T$, let $n_v:=\sum_{u\in V(T_v)}(\log k)^k/(\log(\gamma(u)))^{\gamma(u)}$ where $T_v$ denotes the subtree of $T$ induced by $v$ and all its $T$-descendants.  We say that a node $v$ of $T$ is \emph{heavy} if
    \[
        n_v> \frac{(\log k)^k}{\log\left(c+\frac{\log c}{\log\log c}\right)^{c+\frac{\log c}{\log\log c}}} \enspace ,
    \]
    otherwise $v$ is \emph{light}.
    For a heavy node $v$,
    \begin{align*}
        (\log k)^k/n_v
        & < \log\left(c+\frac{\log c}{\log\log c}\right)^{c+\frac{\log c}{\log\log c}} \\
        & = (\log c)^{\left(c+\frac{\log c}{\log\log c}\right)
            \cdot \left(\tfrac{\log\log (c+\log c/\log\log c)}{\log\log c}\right)} \\
        & < (\log c)^{\left(c+\frac{\log c}{\log\log c}\right)
            \cdot \left(1+\tfrac{(\log^2 e)\log c/\log\log c)}{c\log c}\right)} \\
        & = (\log c)^{\left(c+\frac{\log c}{\log\log c}\right)
            \cdot \left(1+\tfrac{\log^2 e}{c\log\log c}\right)} \\
        & = (\log c)^{c+\frac{\log c}{\log\log c}+\tfrac{\log^2 e}{\log\log c} + \tfrac{(\log^2 e)\log c}{c(\log\log c)^2}} \\
        & = (\log c)^c\cdot c\cdot (\log^2)\cdot(1+o_c(1)) \numberthis \label{sizeish}
    \end{align*}

    Let $T'$ be the subtree of $T$ induced by its heavy nodes.  We say that a node of $T'$ is \emph{special} if it is a leaf or it is a branching node and we let $S$ denote the set of branching nodes.  Together, \cref{heavy-size} and \cref{total-weight-ii,sizeish} imply that the number of leaves of $T'$ is $O(c)$.  In any tree with $\ell$ leaves, the number of branching nodes is at most $\ell-1$.  Therefore the number of special nodes of $T'$ is at most $2c/\alpha$.

    Therefore, for $a\ge 2/\alpha$, one can obtain a humped us-colouring of $T'$ as follows (this is similar to the proof of \cref{skeleton-colour}):  First assign each special node $v\in S\setminus\{r\}$ a distinct colour $\varphi(v)\in\{a(k-c-1)+1,\ldots,ak\}$.  Next, using \cref{pathwidth}, us-colour the graph $T'-S$ (a collection of paths) using colours $\{a(k-c-1)-2,a(k-c-1)-1,a(k-c-1)\}$.

    This gives a colouring $\varphi$ of $T'$ that certainly satisfies (P1).  To see that it also satisfies (P2) observe that, since $|V(T)|\ge 2$, \cref{total-weight-ii} implies that $\gamma(v) > c$ for all $v\in V(T)$.
    Since $\gamma$ is integer valued, this means that $\gamma(v)\ge c+1$.  Therefore, for each $v\in V(T')$, $\varphi(v)\ge a(k-c-1)-2 \ge a(k-c-2)+1\ge a(k-\gamma(v)-1)+1$ so that $\varphi$ satisfies (P2).
