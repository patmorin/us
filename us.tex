...
\begin{lem}\label{two-tree-technical}
    There exists a constant $a>0$ such that, for every integer $k\ge 2$, every $n\le (\log k)^k$, and every $n$-vertex 2-tree $H$, $\uqs(H)\le k$.
\end{lem}

%     eve For every $n$-vertex 2-tree
%     Let $c, k,n$ be integers with $c\ge 2$ and $n\le (\log k)^k$.
%     Let $L_0,\ldots,L_m$ be a BFS layering of some 2-tree $H$ beginning at some edge $v_1v_2$ (so that $L_0=\{v_1,v_2\}$).  For each $i\in\{0,\ldots,m\}$ and each vertex $v$ in $H[L_i]$, let $H_{v}$ be the component of $H[\{v\}\cup\bigcup_{j=i+1}^m L_i]$ that contains $v$ and let $\gamma(v)$ be the maximum integer such that $(\log k)^k/(\log\gamma(vw))^{\gamma(vw)} \ge |H_{vw}|$.
%
%     Fix some connected component $H'$ of $H_{\ge i}:=H[\bigcup_{j=i}^m L_i]$.
%     There exists a constant $a>0$ such that, if
%     \[
%         \sum_{v\in V(H')\cap L_i} \frac{(\log k)^k}{\gamma(v)^{\gamma(v)}} \le \frac{(\log k)}{(\log c)^c} \enspace ,
%     \]
%     then $H'$ has a humped us-colouring $\varphi:V(H')\to\{1,\ldots,ak\}$ such that, for each $\iota\in\{a(k-c-1)+1,\ldots,k\}$, there is at most one vertex $v\in H'\cap L_i$ such that $\varphi(v)=\iota$.
% \end{lem}

\begin{proof}
    Let $v_1,\ldots,v_n$ be a construction order for $H$ and let $(B_x:x\in V(T))$ be a canonical decomposition of $H$ obtained using this order.  We will use $\prec$ to denote the total order on $V(H)$ imposed by the construction order, so that $v_i\prec v_j$ if and only if $i<j$.

    Let $L_0,\ldots,L_m$ be a BFS layering of $H$ beginning at $L_0=\{v_1,v_2\}$.  For each $i\in\{1,\ldots,m\}$, $F_i:=H[L_i]$ is a rooted forest where we use the convention that, for an edge $vw\in E(F_i)$, $v$ is the parent of $w$ if $v\prec w$.

    Let $H_{\ge i}:=H[\bigcup_{j=i}^m L_j]$.  For each edge $vw\in F_i$, let  $H_{vw}$ be the connected component of $H_{\ge i}$ that contains $vw$.  For each vertex $v\in L_i$, let $N_v=\sum_{w\in N_H(v)\cap L_i}|H_{vw}|$.  Our proof is based on the following claim:

    \begin{clm}\label{main-claim}
        Let $H'$ be some connected component of $H_{\ge i}$.  If $\sum_{v\in V(H')} N_v\le (\log k)^k/(\log c)^c$ then $H_{\ge i}$ has a humped us-colouring $\varphi:V(H')\to\{1,\ldots,ak\}$ such that, for each $\iota\in\{a(k-c-1)+1,\ldots,ak\}$ there is at most one vertex $v$ in $V(H')\cap L_i$ with $\varphi(v)=\iota$.
    \end{clm}

    \begin{proof}[Proof of \cref{main-claim}]
        The proof is by induction on $m-i$.  In the base case $m-i=0$ and $H'$ is a tree.  For this case, we can easily use \cref{tree-ii} by setting $\gamma(v)=1$ for each $v\in V(H')$, setting $\phi=k$ and then applying \cref{tree-ii}.

        In the general case, we set $\gamma(v):=\max\{c\in\N:(\log k)^k/c^c \ge N_v\}$ for each $v\in V(H)$. Then By \cref{tree-ii}, we can find a us-colouring $\varphi$ of the tree $H'[L_i]$ with the desired properties and such taht $\varphi(v)\ge a(k-c-\gamma(v)-1)+1$ for all $v\in V(H'[L_i])$.  It remains to show that we can extend this to a colouring of $V(H')\cap L_{i+1}\cap\cdots\cap L_m$.  This extension requires some care.

        For the root $r$ of the tree $H'[L_i]$, we define $D_r=\{x\in L_{i+1}:rx\in E(H)\}$.  For each node $w$ with parent $v$ in $H'[L_i]$, we define $D_w=\{x\in L_{i+1}:wx\in E(H)\}\setminus D_{v}$.  Fix any $w\in V(H')\cap L_i$ and consider the graph $H'':=H'[D_w\cup \bigcup_{j=2}^m L_j]$.  Now, $H''[L_{i+1}]$ is a rooted forest with roots $w_1,\ldots,w_d$.  We add a node $w'$ to $H''$ to obtain a graph on which we can apply induction.  This induction yields a humped us-colouring of $H'$
