...
\begin{proof}
    ...
    \begin{compactenum}
       ...
       \item $\{w_a,w_b\}\in V(G_j')$ for some $j\in\{1,\ldots,p\}$. In this case, edge maximality implies that $B_{y_{j-1}}$ and $B_{y_{j}}$ each form cliques.  Since $w_a,\ldots,w_b$ is induced, this implies that $\{w_a,\ldots,w_b\}\subseteq V(G_j')\cup\{u_{j-1}\}$.  By edge maximality, the vertex $u_{j-1}$ dominates $V(G_j')$, so if $u_{j-1}\in\{w_a,\ldots,w_b\}$, then $b=a+2$ and $w_{a+1}=u_{j-1}\in U$ and we are done.  Otherwise, $w_a,\ldots,w_b$ is an induced path in $G_j'$ so, by the inductive hypothesis, $w_{a+1},\ldots,w_{b-1}\subseteq U_j'\subseteq U$.  (Note that this includes the special case in which $w_a=u_j$ and $w_b\in V(G_j')$.)
