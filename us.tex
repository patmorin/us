...
\begin{proof}
    We say that node $v$ of $T$ is \emph{heavy} if
    \[
        W(v)> \frac{(\log k)^k}{(\log(c+1+\tfrac{\log (c+1)}{\log\log (c+1)}))^{c+1+\tfrac{\log (c+1)}{\log\log(c+1)}}} \enspace .
    \]
    ...
    Let $T'$ be the subtree of $T$ induced by the set of heavy nodes.  Since no leaf of $T'$ is a $T$-ancestor of any other leaf, it follows that the number of leaves of $T'$ is at most $(3c(c-1)\log^2 e)/2= O(c^2)$.  Therefore $T'$ has a us-colouring $\varphi':V(T')\to \{a(k-c)-r,\ldots,a(k-c)\}\setminus\{\phi\}$ for some $r\in O(\log c/\log\log c)$.  We then set $\varphi(r)=\phi$ and, for each $v\in V(T')\setminus\{r\}$ we set $\varphi(v):=\varphi'(v)$.  This gives a us-colour of $T'$$ to the colour of


    .  We therefore compute a colouring of $T$, and we do so by first colouring $r$ with $\varphi(r):=\phi$ and colouring the remaining nodes of $T'$ with the colours in $\{a(k-c)-\beta\log c/\log\log c,\ldots,a(k-c)\}\setminus\{\phi\}$.  Note that this partial colouring satisifies requirements (P1) and (P2). In particular, for each $v\in V(T)$,
    \[
       \frac{(\log k)^k}{\gamma(v)^{\gamma(v)}} \le W(r) \le \frac{(\log k)^k}{\left(\log c+\tfrac{\log c}{\log\log c}\right)^{c+\tfrac{\log c}{\log\log c}}}
   \]
   ...
   \[
       W(r_i) \le \frac{(\log k)^k}{(\log(c+1+\tfrac{\log (c+1)}{\log\log (c+1)}))^{c+1+\tfrac{\log (c+1)}{\log\log(c+1)}}} \enspace .
   \]
   Therefore, we can apply induction on each tree $T_i$ using the colour $\phi_i=a(k-c-)



   \[
        \frac{(\log k)^k}{\gamma(v)^{\gamma(v)}}\le W(r_i) < \frac{(\log k)^k}{\left(\log c+\tfrac{\log c}{\log\log c}\right)^{c+\tfrac{\log c}{\log\log c}}}
   \]
   Therefore $\gamma(v) > \gamma(r)$. Since $\gamma(r)$ and $\gamma(r_i)$ are integers, this implies that
   \[  \gamma(r_i) \ge \gamma(r)+1 \ge c + \frac{\log c}{\log\log c} + 1 \]
   Therefore, we may choose $\phi_i = a(k-c)-\beta\log c/\log\log c-1\ge$
...
\begin{compactenum}[(R1)]
    % \item $\gamma(v)\le k-c$ for each $v\in V(T)$; and \label{min-weight}
...
\begin{proof}
    ...
    % \end{compactenum}
    % % In order to apply induction, What remains is to argue that these satisfy the preconditions 1 and 2 of the lemma.
    ...
    %
    %
    % The proof is by induction on $|T|$.  In the base case, $T$ consists of a single node $r$ and $w(r)\le k-c$. Therefore $\phi \ge a(k-c-1) \ge a(w(r)-1)$.    Setting $\varphi(r):= a(k-w(r)) \le a(k-c)$ gives the required colouring.
    %
    % For $|T|>1$, consider the root $r$ of $T$, let $r_1,\ldots,r_d$ be the children of $r$ and, for each $i\in\{1,\ldots,d\}$, let $T_i$ be the subtree of $T$ induced by $r_i$ and all its $T$-descendants.  We say that $T_i$ is \emph{heavy} if $\sum_{v\in V(T_i)} (\log w(v))^{w(v)} \ge (\log k)^k/(\log (c+1))^{c+1}$.  LOOKOUT: Deal with $c=0$ separately. By \cref{total-weight}, the number of heavy subtrees is at most
    % \begin{align*}
    %     d' & \le \frac{(\log k)^k}{(\log c)^c} \bigg/ \frac{(\log k)^k}{(\log(c+1))^{c+1}} \\
    %     &= \frac{(\log(c+1))^{c+1}}{(\log c)^c} \\
    %     &= \left(\frac{\log(c+1)}{\log c}\right)^c\times \log(c+1) \\
    %     &\le \left(\frac{\log(c+1)}{\log (c+1)-(\log e)/c}\right)^c\times \log(c+1) \\
    %     &\le \left(1+\frac{2\log e}{c}\right)^c\times \log(c+1)
    %         & \text{when $c\log(c+1)\ge 2\log e$ } \\
    %     &\le \left(e^{(2\log e)/c}\right)^c\times \log(c+1) \\
    %     &\le e^{2\log e}\times \log(c+1) \\
    % \end{align*}
    %
    % We say that a subtree $T_i$ is \emph{heavy} if $W(T_i)> W(T)/(ac+1)$ and is light otherwise.  Observe that there are at most $ac$ heavy subtrees.  For each light subtree $T_i$, we have
    % \begin{align*}
    %     W(T_i) & \le W(T)/c \\
    %     & \le \frac{(\log k)^k}{c(\log c)^c} \\
    %     & = \frac{(\log k)^k}{(\log(c+r))^{\tfrac{\log (ac+1)}{\log\log(c+r)}}(\log c)^c} \\
    %     & = \frac{(\log k)^k}{(\log(c+r))^{\tfrac{\log (ac+1) + c\log\log c}{\log\log(c+r)}}} \\
    %     & = \frac{(\log k)^k}{(\log(c+r))^{c+\tfrac{\log (ac+1)}{\log\log(c+r)} - O(r/\log c)}}  &\text{(this step is explained below)}\\
    %     & \le \frac{(\log k)^k}{(\log(c+r))^{c+\tfrac{\log c}{\log\log(c+r)} + \log(a)/\log\log(c+r) - O(r/\log c)}} \\
    %     & \le \frac{(\log k)^k}{(\log(c+r))^{c+\tfrac{\log c}{\log\log(c+r)}}}
    %     & \text{(for large $a$ and $r\in O(\log c/\log\log c))$} \\
    %     % & = \frac{(\log k)^k}{(\log(c+r))^{\log (ac+1)/\log\log(c+r) + c\log\log(c)} \\
    %     & = \frac{(\log k)^k}{(\log(c+\alpha\log c/\log^{(3)} c))^{c+\tfrac{\alpha \log c}{\log^{(3)}c}}}
    % \end{align*}
    % where the last step comes from solving $r=\log c/\log\log (c+r)$, who solution is $\alpha\log c/\log^{(3)}c$ for some $\alpha\in[??]$.
    %
