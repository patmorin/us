...
\begin{proof}
    ...
    If every node $x$ of $T$ has $|B_x|= t+1$, then we are done; $\mathcal{T}$ is a canonical tree decomposition of a simple $t$-tree $H$.
    ...
    We say that $\mathcal{T}$ is \emph{uniform} if there is no node $x$ with child $y$ such that $|B_y\setminus B_x|>1$.  Suppose $\mathcal{T}$ is not uniform and that $B_y\setminus B_x=v_1,\ldots,v_d$ for some parent-child edge $xy\in E(T)$.  If $|B_y|\le t$, then we can replace the edge $xy$ with a path $x,x_1,\ldots,x_{d-1},y$ where, for each $i\in\{1,\ldots,d\}$, $B_{x_i}=\{v_1,\ldots,v_i\}\cup (B_x\cap B_y)$.  This does not affect $t$-simplicity since each newly introduced node $x_i$ has $|B_{x_i}|<|B_y|\le t$.

    If $|B_y|=t+1$ and there is no child $z$ of $y$ such that $B_z\supseteq\{v_1,\ldots,v_{d-1}\}\cup (B_x\cap B_y)$, then we perform the same operation, replacing $xy$ with $x,x_1,\ldots,x_{d-1},y$.  Again, this does not affect $t$-simplicity since $|B_{x_i}|<t$ for each $i\in\{1,\ldots,d-2\}$ and the the $t$-element subset $B_{x_{d-1}}$ appears is contained only in $B_{x_{d-1}}$ and $B_y$.
