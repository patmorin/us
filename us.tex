...
\begin{proof}[Proof of \cref{simple-t-trees} (upper bound)]
    ...
    \[  (\log^{(t-2)} k)^{k} \ge n 
        ...
        \Leftrightarrow
        \frac{k\log^{(t-1)} k}{\log n} \ge 1
    \]
    Taking $k=2\log n/\log^{(t)} n$, we get
    \begin{align*}
      \frac{k\log^{(t-1)} k}{\log n}
      & = \frac{2\log^{(t-1)} k}{\log^{(t)} n} \\
      & = \frac{2\log^{(t-1)}\left(2\log n/\log^{(t)} n\right)}{\log^{(t)} n} \\
      & \ge \frac{2\log^{(t-1)}\left(\log n/\log^{(t)} n\right)}{\log^{(t)} n} \\
      & = \frac{2\log^{(t-2)}\left(\log^{(2)} n - \log^{(t+1)} n\right)}{\log^{(t)} n} \\
      & \approx \frac{2\log^{(t-3)} \left(\log^{(3)} n - \tfrac{\log^{(t+1)} n}{\log^{(2)} n}\right)}{\log^{(t)} n} & \text{because $x-a=x(1-a/x)\approx xe^{-a/x}$} \\
      & \approx \frac{2\log^{(t)} n - \tfrac{\log^{(t+1)} n}{\prod_{j=2}^{t-1}\log^{(j)} n}}{\log^{(t)} n} & \text{by iteration} \\
    \end{align*}
    % \[
    % 
    % \]
