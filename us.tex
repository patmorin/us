...
The following lemma is due to \citet{pilipczuk.siebertz:polynomial}.
\begin{lem}[\cite{pilipczuk.siebertz:polynomial}]
    Let $H$ be a $t$-tree with construction order $\mathcal{R}$.  Then, for each integer $\ell\ge 0$ and each $u_0\in V(H)$, the number of paths $u_0,\ldots,u_r$ in $H$ such that $r\le\ell$ and $u_0>_\mathcal{R} \cdots>_\mathcal{R} u_r$ is at most $\binom{\ell+t}{t}$.
\end{lem}
...
A node $x$ in a rooted tree $T$ is a \emph{branching node} if $x$ has at least two children.  Let $\Lambda(T)$ denote the set of branching nodes in a tree $T$.  Let $H$ be a $t$-tree with a construction order $\mathcal{R}$ that generates a canonical tree decomposition $\mathcal{T}:=(B_x:x\in V(T))$.  Define vertex sets $V_0\subseteq V_1\subseteq\cdots\subseteq V_t$ as follows:
\begin{itemize}
    \item $V_0:=\bigcup_{x\in\Lambda(T)} B_x$.
    \item For each $i\in\{1,\ldots,t\}$, $V_i$ contains every vertex of every path $w_0,\ldots,w_q$ such that
    \begin{compactenum}
        \item $\{w_0,w_q\}\subseteq V_{i-1}$;
        \item $q\in\{0,\ldots,\ell\}$; and
        \item $w_0\prec_\mathcal{T} w_1\prec_{\mathcal{T}} \cdots \prec_\mathcal{T} w_q$.
    \end{compactenum}
\end{itemize}

\begin{lem}
    There exists a function $f:\N^2\to\N$ such that
    $|V_t|\le f(t,\ell)\cdot |\Lambda(T)|
\end{lem}



We define the $(\mathcal{T},\ell)$-skeleton $\hat{H}$ of $H$ as the subgraph of $H$ induced by each vertex $w_r$ such that
\begin{inparalist}[(i)]
    \item $r\le\ell$,
    \item $w_0,\ldots,w_r$ is a path in $H$ with $w_0>_\mathcal{R}\cdots >_\mathcal{R} w_r$;
    \item $w_0\in \bigcup_{x\in\Lamba(T)} B_x$.
\end{enumerate}

% ; and
     % \item the \emph{closed $\mathcal{T}$-skeleton} $\hat{H}^+$ with vertex set $V(\hat{H}_\mathcal{T}^+):=V(\hat{H}_\mathcal{T})$ and that contains an edge $vw$ if and only if there exists $x,y\in \Lambda(T)$ with $v\in B_x$, $w\in B_y$ and the path in $T$ from $x$ to $y$ contains no other branching node of $T$.
 % \end{itemize}
 % In words, $H[S_T]^+$ contains every path between every pair of vertices $v,w\in V(H)$.
 % It is worth noting that $\hat{H}_\mathcal{T}$ is a subgraph of $H$ so $\stw(\hat{H}) \le \stw(H)$, but this is not necessarily the case for $\hat{H}^+$.  Indeed, $\hat{H}^+$ may contain a complete bipartite graphs of size $2(t+1)$, which has treewidth $t+1$.
...
\end{lem}
 ...
 % we define the $\mathcal{T}$-skeleton $\hat{H}$ of $H$ as the induced subgraph $H[\bigcup_{x\in \Lambda(T)} B_x]$.

% and containing the edge $vw$ if there exists $x,y\in V(T)$ with $v\in B_x$, $w\in B_y$ such that the path in $T$ from $x$ to $y$ contains no branching node in its interior.  [TODO: Absolutely not true!] It is worth noting that the skeleton $\hat{H}$ of $H$ is a minor of $H$ since it can be obtained from $H$ by repeatedly contracting an edge $vw$ with $v\in V(\hat{H})$ and $w\not\in V(\hat{H})$ into $v$. By \cref{simple-minor-closed} this implies that, $\stw(\hat{H})\le\stw(H)$.
...
% \begin{lem}\label{skeleton-colour}
%     Let $H$ be a graph having a width-$t$ tree decomposition $\mathcal{T}:=(B_x:x\in V(T))$, and let $\hat{H}$ be the $\mathcal{T}$-skeleton of $H$.  Then $\lrn(H)\le 2(t+1)\lrn(\hat{H}) + (\ell+1)t+1$.
% \end{lem}
...
\begin{proof}
    ...
    \begin{compactenum}
        ...
        \begin{compactenum}
            \item $\{u_1,\ldots,u_r\}\subseteq V(H-V(\hat{H}))$.  In this case, $u_0,\ldots,r_r$ is a path in $H-V(\hat{H})$, so $\varphi(u_0)\neq \varphi(u_r)$ or $\varphi(u_0)<\varphi(u_i)$ for some $i\in\{1,\ldots,r-1\}$ since $\varphi$ is an $\ell$-ranking of $H-V(\hat{H})$.

            \item $v_i\in V(\hat{H})$ for some $i\in\{1,\ldots,r\}$. In this case, $\varphi(u_0)\le 3t+1 < \varphi(u_i)$.
        \end{compactenum}
        \item $\{u_0,u_r\}\subseteq V(\hat{H})$. We claim that, in this case $u_1,\ldots,u_r\in V(\hat{H})$. Otherwise, there exists $0\le i\le j-2\le r$ such that $\{u_i,u_j\}\subseteq V(\hat{H})$ but $\{u_{i+1},\ldots,u_{j-1}\subseteq V(H-\hat{H})$.  Since $u_i\in V(\hat{H})$, there exists a branching node $x$ of $T$ and a path $w_0,\ldots,w_p$ such that $w\in B_x$ and $w_0>_\mathcal{R}\cdots>_\mathcal{R} w_p$.
