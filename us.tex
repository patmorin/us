...
The recursive definition of $t$-trees implies that there is a permutation $v_1,\ldots,v_n$ of $V(H)$ such that, for each $i\in\{1,\ldots,n\}$, $H[N_H(v_i)\cap \{v_1,\ldots,v_{i-1}\}]$ is a clique of size.  We call $v_1,\ldots,v_n$ a \emph{construction order} for $H$.  For a given construction order $v_1,\ldots,v_n$ of $H$, we call $C_{v_i}:=N_H(v_i)\cap \{v_1,\ldots,v_{i-1}\}$ the \emph{parent clique} of $v_i$ (with respect to the order $v_1,\ldots,v_n$).  The \emph{dominant parent} $p(v_i)$ of $v_i$ (w.r.t. $v_1,\ldots,v_n$) is the vertex $v_j\in C_{v_i}$ that minimizes $j$. Note that the parent clique and dominant parent of any vertex $v_i$, is determined by the choice of the \emph{root clique} $v_1,\ldots,v_{t}$.

If $v_1,\ldots,v_n$ is a construction order for a $t$-tree $H$ and $L_0,\ldots,L_m$ is a BFS layering of $H$ with $L_0:=\{v_1,\ldots,v_{t}\}$, then for any any $v_i\in L_{i'}$ and $v_j\in L_{j'}$, $i < j$ implies $i'\le j'$.  We will use this property frequently.  In particular, if $v\in L_{i}$ for some $i\in\{1,\ldots,m\}$, then the dominant parent $p(v)\in L_{i-1}$.  We also make use of the following property:

\begin{lem}\label{up-clique}
    Let $H$ be a $t$-tree with construction order $v_1,\ldots,v_n$ and let $L_0,\ldots,L_m$ be a BFS layering of $H$ with $L_0:=\{v_1,\ldots,v_{t+1}\}$.  For any $i\in\{0,\ldots,m-1\}$ and any path $uvw$ in $H$ with $u,w\in L_{i+1}$, and $v\in L_i$, there exists a $t$-clique $C$ in $H[L_i]$ such that $(N_H(u)\cup N_H(w))\cap L_i\subseteq C$.
\end{lem}
...
Our proof is based on the following claim:
    ...
    \begin{proof}[Proof of \cref{main-claim}]
       ...
       \begin{compactenum}
          ...
          \item If $u,w\in L_i$ and $v\in L_{i+1}$ then $u,w$ is contained the parent clique $C_v$ of $v$.  In particular, $uw\in E(H'[L_i])$ so $\varphi(u)\neq\varphi(w)$ since \cref{t-tree-slack} produces a 2-ranking (which is a proper colouring) $\varphi$ of $H'[L_i]$.
          ...
          \item If $u,w\in V(H_{p}-H^-_{p})$ for some $p\in V(H'[L_i])$ then $\varphi(u)$ and $\psi(w)$ were assigned by some vertex $p'\in V(H'[L_i])$ such that $v\in S_{p'}$.  Similarly, $\varphi(w)$ and $\psi(w)$ were assigned by some vertex $q'\in V(H'[L_i])$ such that $w\in S_{q'}$.  There are two subcases to consider:
          \begin{compactenum}
            \item If $p'=q'$ then $\varphi(u)\neq\varphi(q)$ since every vertex in $S_{p'}$ is assigned a unique colour by $\varphi$.
            \item If $p'\neq q'$ then, by \cref{up-clique}, $p'q'\in E(H'[L_i])$ so that $\psi(u)\neq \psi(w)$.
        \end{compactenum}
        In either case $\theta(x)\neq\theta(z)$.

        \item If $u\in V(H^-_{p})$ and $w\in V(H_p-H^-_{p})$ for some $p\in V(H'[L_i])$ then $w\in S_{q'}$ for some $q'\in V(H'[L_i])$ and $\varphi(w)$ (and $\psi(w)$) was assigned by $q'$, so $\varphi(w)>a(k-\gamma(q')-1)$. On the other hand, by \cref{min}, $\varphi(u) \le a(k-\gamma(q')-1)$.  Therefore $\varphi(u)<\varphi(w)$ so $\theta(u)\neq\theta(w)$.
      \end{compactenum}
