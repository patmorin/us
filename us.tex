...
\begin{proof}
    ...
    For each vertex $v\in L_i$, let $N_i(v)$ denote the set of $t$-cliques in $H[L_i]$ that contain $v$ and let $H^+_v:=H[\bigcup_{C\in N_i(v)} H_C]-L_i$.  In words, $H^+_v$ is the set of all $t$-trees hanging off all the $t$-cliques of $H_i$ that contain $v$.  Let $n_v:=|H^+_v|$.

    \begin{clm}\label{size-claim}
        For each $i\in\{1,\ldots,m\}$ and each component $H'$ of $H[\bigcup_{j=i}^m L_j]$, $\sum_{v\in V(H'[L_i])} |H^+_v| \le t|H'-L_i|$.
    \end{clm}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            & = \sum_{v\in V(H'[L_i])}|H^+_v|/t \enspace . \qedhere
        \end{align*}
    \end{proof}
