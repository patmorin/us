...
\begin{proof}
    For the sake of contradiction, suppose otherwise.  First note that $x_T:V(H)\to V(T)$ is bijective, except for vertices in $B_r$.  Next observe that, for any edge $u_iu_{i+1}\in E(H)$, $x_T(u_i)\preceq_T x_T(u_{i+1})$ or $x_T(u_{i+1})\preceq_T x_T(u_{i})$.  There are a few cases to consider:
    \begin{enumerate}
...
A node $x$ in a rooted tree $T$ is a \emph{branching node} if $x$ has at least two children.  Let $\Lambda(T)$ denote the set of branching nodes in a tree $T$.  Let $H$ be a $t$-tree with a construction order $\mathcal{R}$ that generates a canonical tree decomposition $\mathcal{T}:=(B_x:x\in V(T))$.  Define a sequence of pairwise-disjoint vertex sets $V_0,\ldots V_t$ as follows:
\begin{itemize}
    \item $V_0:=\bigcup_{x\in\Lambda(T)} B_x$.

    \item For each $i\in\{1,\ldots,t\}$, $V_i$ contains $w_1,\ldots,w_{q-1}$ for each $\mathcal{T}$-downward path $w_0,\ldots,w_q$ such that $q\le\ell$; $\{w_0,w_q\}\subseteq \bigcup_{j=0}^{i-1} V_j$; and $w_1,\ldots,w_{q-1}\cap \bigcup_{j=0}^{i-1} V_i=\emptyset$
\end{itemize}
We call $H[\bigcup_{i=0}^t V_t]$ the $(\mathcal{T},\ell)$-skeleton of $H$ and we will always denote it by $\hat{H}$.

\begin{lem}\label{skeleton-size}
    $|\bigcup_{j=0}^t V_t|\le \binom{t+\ell}{t}^t\cdot (t+1)\cdot|\Lambda(T)|$.
\end{lem}

\begin{proof}
    By \cref{upward-count}, $|\bigcup_{j=0}^{i} V_j|\le \binom{t+\ell}{t}\cdot |\bigcup_{j=0}^{i-1} V_{j}|$, so $|\bigcup_{j=0}^{t} V_j|\le \binom{t+\ell}{t}^t\cdot|V_0| \le \binom{t+\ell}{t}^t\cdot (t+1)\cdot|\Lambda(T)|$.
\end{proof}

\begin{lem}\label{skeleton-paths}
    Let $w_0,\ldots,w_q$ be an induced $\mathcal{T}$-downward path in $H$ of length at most $\ell$ and with endpoints $\{u_0,u_r\}\subseteq V_t$. Then $\{u_1,\ldots,u_{r-1}\}\subseteq V_t$.
\end{lem}

\begin{proof}
    % Let $H_0:=H[V_0]$ and, for each $i\in\{1,\ldots,t+1\}$, let $H_i$ denote the subgraph of $H$ obtained by removing
    % \begin{enumerate}
    %     \item every edge of $H[V_{i-1}]$;
    %
    %     \item any vertex $v\in V(H-V_{i-1})$ that is not part of some induced $\mathcal{T}$-upward path, in $H$, of length at most $\ell$ with endpoints in $V_{i-1}$.
    % \end{enumerate}
    % By definition, $V(H_i)=V_i$.

    Let $w_0,\ldots,w_q$ be a path satisfying the second criteria.
    ...
    and let $x_0,\ldots,x_q:=x_T(u_0),\ldots,x_T(u_q)$.   We claim that, for each $i\in\{1,\ldots,q-1\}$, $|B_{x_i}\setminus \bigcup_{j=0}^{i-1} V_{j}|\le t+2-i$ and prove this by induction on $i$.  The base case $i=1$ is trivial, since $\mathcal{T}$ is a width-$t$ decomposition, so $B_x\le t+1$ for each $x\in V(T)$.

    Now assume $i\ge 2$.  Since $w_0,w_q\in \bigcup_{j=0}^{i-1} V_{j}$ but $w,
...
\begin{proof}
    ...
    \begin{compactenum}
        ...
        \end{compactenum}
        \item $\{u_0,u_r\}\subseteq V(\hat{H})$. In this case, \cref{induced-unimodal} implies that $u_0,\ldots,u_r$ is $\mathcal{T}$-unimodal, so  \cref{skeleton-paths} implies that $u_0,\ldots,u_r\in V(\hat{H})$.
        % Since $u_0,\ldots,u_r$ is an induced path in $H$, \cref{induced-unimodal} implies that there exists $m\in\{0,\ldots,r-1\}$ such that $u_0,\ldots,u_m$ is $\mathcal{T}$-upward and $u_{m+1},\ldots,u_m$ is $\mathcal{T}$-downward. If $m\not\in\{0,m-1\}$, then $x_T(u_m)$ is a branching node of $T$, so $u_m\in V_0\subeteq V_{t-1}$.
        Therefore $u_0,\ldots,r_r$ is a path in $\hat{H}$, so  $\varphi(u_0)<\varphi(u_i)$ for some $i\in\{1,\ldots,r-1\}$ since $\varphi$ is an $\ell$-ranking of $\hat{H}$.
        %
        %   Indeed,
        %
        %
        % Since $u_0,\ldots,u_r$ is an induced path in $H$, \cref{induced-unimodal} implies that there exists $m\in\{0,\ldots,r-1\}$ such that $u_0,\ldots,u_m$ is $\mathcal{T}$-upward and $u_{m+1},\ldots,u_m$ is $\mathcal{T}$-downward. If $m\not\in\{0,m-1\}$, then $x_T(u_m)$ is a branching node of $T$,
        %
        %
        %
        %
        % This follows from the next claim and \cref{upward-paths}.
        % To see this, first observe that, because $u_0,\ldots,u_r$ is an induced path, $u_0,\ldots,u_r$ is \emph{$\mathcal{T}$-unimodal}, i.e., There exists $m\in\{1,\ldots,r\}$ such that $u_0,\ldots,u_m$ is $\mathcal{T}$-upward and $u_m,\ldots,u_r$ is $\mathcal{T}$-downward.
        % %
        %
        % $u_m\prec_\mathcal{T} u_{m-1}\prec_\mathcal{T}\cdots\prec_\mathcal{T} u_0$ and $u_m\prec_{T}u_{m+1}\prec_\mathcal{T}\cdots\prec_\mathcal{T} u_r$.

        %
        % Otherwise, there exists $0\le i\le j-2\le r$ such that $\{u_i,u_j\}\subseteq V(\hat{H})$ but $\{u_{i+1},\ldots,u_{j-1}\subseteq V(H-\hat{H})$.  Since $u_i\in V(\hat{H})$, there exists a branching node $x$ of $T$ and a path $w_0,\ldots,w_p$ such that $w\in B_x$ and $w_0>_\mathcal{R}\cdots>_\mathcal{R} w_p$.
        %
        %
        % some $u\in B_x$ such that $u_0
        %
        %
        % Consider the sequence $x_i,\ldots,x_j:=x_T(u_i),\ldots,x_T(u_j)$.
        %
        %
        %
        % To see this, consider the sequence of nodes $x_0,\ldots,x_r:=x_T(u_0),\ldots,x_T(u_r)$.
        %
        %
        % Since $u_0$ and $u_r$ are in $V(\hat{H})$,
    \end{compactenum}
\end{proof}
