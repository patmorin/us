...
\begin{equation}
     n_H := \sum_{v\in V(H)} n_v \le \frac{(\log^{(t-2)} k)^k}{\left(\log^{(t-2)} (c + s)\right)^{c+s}} \enspace ,
...
\begin{proof}
    ...
    Next, we apply induction on $H_C$ for each maximal clique $C$ in $I$.  Let us verify that this is possible.  If $C$ is a dangerous clique, then $\varphi(v)>a(k-c-1)$ for each $v\in C$ and $|H_C|\le |H|\le(\log^{(t-2)}k)^k/(\log^{(t-2)}(c+s))^{c+s}$.  
