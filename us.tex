...
\begin{lem}\label{t-tree-no-slack-separator}
    ...
    \end{equation}
    then there exists $S\subseteq V(T)$ such that
    \begin{compactenum}
        \item $|S|\in O(c+1)$; and
        \item For each component $X$ of $H-\bigcup_{x\in S} B_x$,
        \[
...
\begin{proof}
    ...
    It is well-known, and easy to prove, that there exists a set $S\subseteq V(T)$ of size at most
    \[
        \left\lceil \frac{W(H)}{W_0}\right\rceil \in O(c)
    \]
    such that each component of $H-\bigcup_{x\in S}B_x$ is light, which completes the proof.
    %
    %
    %
