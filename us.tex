...
% A sequence of integers $\phi_0,\ldots,\phi_r$ is \emph{ranked} if $\max\{\phi_0,\ldots,\phi_r\}$ occurs exactly once in $\phi_0,\ldots,\phi_r$.
...
% A path $P:=u_0,\ldots,u_r$ in $G$ is an \emph{induced} path if $G[\{u_0,\ldots,u_r\}]$ is a path.
...
% A \emph{$t$-forest} is a graph whose connected components are $t$-trees.
...
% The \emph{dominant parent} of $v_i$ (w.r.t. $v_1,\ldots,v_n$) is the vertex $v_j\in C_{v_i}$ that minimizes $j$. Note that the parent clique and dominant parent of any vertex $v_i$, is determined by the choice of the \emph{root clique} $v_1,\ldots,v_{t}$.
...
% For any $X$-decomposition of $G$, it is straightforward, and common, to obtain a smooth $X'$-decomposition of $G$ where $X'$ is a graph obtained by subdividing edges of $X$.
...
\begin{obs}\label{induced-unimodal}
    ...
    % Let $H$ be a $t$-tree with a construction order $\mathcal{R}$ generating a canonical tree decomposition $\mathcal{T}$ and let $u_0,\ldots,u_r$ be an induced path in $H$.  Then $u_0,\ldots,u_r$ is $\mathcal{T}$-unimodal.
\end{obs}
...
%
% \begin{proof}
%     For the sake of contradiction, suppose otherwise.  First note that $x_T:V(H)\to V(T)$ is \emph{nearly bijective} in the sense that for distinct $v,w\in V(H)$, $x_T(v)\neq x_T(w)$ or $\{v,w\}\subseteq B_r$. In the latter case $x_T(v)=x_T(w)=r$.  Next observe that, for any edge $u_iu_{i+1}\in E(H)$, $x_T(u_i)\preceq_T x_T(u_{i+1})$ or $x_T(u_{i+1})\preceq_T x_T(u_{i})$.  There are a few cases to consider:
%     \begin{enumerate}
...
%
%         \item There exists $i\in\{1,\ldots,r-1\}$ such that $x_T(u_{i-1})\prec_\mathcal{T} x_T(u_i)= x_T(u_{i+1})$.  Since $x_T$ is nearly bijective, this implies that $x_T(u_{i})=x_T(u_{i+1})=r$.  But this contradicts $x_T(u_{i-1})\prec_\mathcal{T} x_T(u_i)$ since $r\prec_T x$ for every $x\in V(T)\setminus\{r\}$.
%
...
%         $x_T(u_{i+1})\prec_T x_T(u_{i-1}) \prec_T(u_{i})$.  In the former case, Property~(ii) of tree decompositions implies that $u_{i-1}\in B_{x_T(u_{i+1})}$.  In the latter case, $u_{i+1}\in B_{x_T(u_{i-1})}$.  Both cases imply that $u_{i-1}u_{i+1}\in E(H)$, contradicting the fact that $u_0,\ldots,u_r$ is an induced path of $H$.
%     \end{enumerate}
...
\begin{proof}
    ...
    Since $\rho$ is an $\ell$-ranking of $G_1$, \cref{walks-too} implies that $\rho(u_{0,1})<\max\{\rho_{u_0,1},\ldots,u_{r,1}\}$ and therefore $\varphi(u)<\max\{\varphi(u_0),\ldots,\varphi(u_r)\}$, as required.
      % Therefore $G_1[\{u_{0,1},\ldots,u_{r,1}\}]$ contains a path $w_0,\ldots,w_s$ with $w_0=u_{0,1}$, $w_s=u_{r,1}$, and $\{w_1,\ldots,w_{s-1}\}\subseteq \{u_{1,1},\ldots,u_{r,1}\}$. Since $\varphi(u_0)=\varphi(u_1)$, $\psi(w_0)=\psi(u_{0,1})=\psi(u_r,1)=\psi(w_s)$.
...
\begin{proof}
    ...
    \begin{compactenum}
        ...
        % In this case, \cref{induced-unimodal} implies that $u_0,\ldots,u_r$ is $\mathcal{T}$-unimodal, i.e., $u_0,\ldots,u_r$ is the concatenation of two paths, one upward and one downward.
...
% The following lemma gives criteria under which the first few

\noindent[TODO: Do we ever use $g$ anymore?]
...
Let $H$ be a simple $t$-tree, let $v_1,\ldots,v_n$ be a construction order for $H$ and let $L_0,\ldots,L_m$ be a BFS layering of $H$ with $L_0=\{v_1,\ldots,v_{t}\}$.  For each $i\in\{0,\ldots,m\}$ and each $t$-clique $C$ in $H[L_i]$, we use $H_C$ to denote the connected component of $H[C\cup\bigcup_{j=i+1}^m L_j]$ that contains $C$.  For each vertex $v\in L_i$, let $N_i(v)$ denote the set of $t$-cliques in $H[L_i]$ that contain $v$.
 ...
 % and let $H^+_v:=H[\bigcup_{C\in N_i(v)} H_C]-L_i$.  In words, $H^+_v$ is the set of all $t$-trees hanging off all the $t$-cliques of $H[L_i]$ that contain $v$.
% Let $n_v:=1+|H^+_v|$.
