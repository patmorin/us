...
We use a BFS layering $L_1,\ldots,L_m$ of $H$ and use the fact (\cref{simple-bfs-layers}) that each layer $L_i$ induces a $(t-2)$-forest $H[L_i]$, on which we can apply \weirdref{simple-t-trees}{a} inductively.\footnote{Here, and throughout, we make use of the fact $\trn(H)$ is monotone: it does not decrease when edges are added to $H$ and the fact (\cref{simple-subgraph}) that any graph $H$ is a subgraph of some simple $t$-tree $H'$ with $V(H')=V(H)$ and $E(H')\supseteq E(H)$.}  The main difficulty with this approach is that a vertex $v$ in $H[L_i]$ dominates an entire simple $(t-2)$-forest $F$ in $H[L_{i+1}]$.  If some colour $\phi$ is used more than once in the colouring of $F$, then $v$ must be assigned a colour larger than $\phi$.
...
Our proof is based on the following claim:
    ...
    \begin{proof}[Proof of \cref{main-claim}]
        ...
        In the general case, we set $\gamma(v):=\max\{x\in\N:(\log^{(t-2)} k)^k/(\log^{(t-2)} x)^x \ge n_v\}$ for each $v\in V(H'[L_i])$.  This implies that
        \[
            \frac{(\log^{(t-2)} k)^k}{(\log^{(t-2)}(\gamma(v)+1))^{\gamma(v)+1}} < n_v
        \]
        Since $\gamma(v)+1\le k$ [TODO: Explain!],
        \[
           n_v > \frac{(\log^{(t-2)} k)^k}{(\log^{(t-2)}(\gamma(v)+1))^{\gamma(v)+1}}
           \ge \frac{(\log^{(t-2)} k)^{k-1}}{(\log^{(t-2)}(\gamma(v)+1))^{\gamma(v)}}
            \cdot
            \frac{\log^{(t-2)} k}{\log^{(t-2)}(\gamma(v)+1)}
           \ge
           \frac{(\log^{(t-2)} k)^{k-1}}{(\log^{(t-2)}\gamma(v))^{\gamma(v)}}
       \]
       Therefore,
       \[
           \sum_{v\in H'[L_i]} \frac{(\log^{(t-2)} k)^k}{(\log^{(t-2)} \gamma(v))^{\gamma(v)}} < \sum_{v\in H'[L_i]} n_v(\log k)
           \le \frac{(\log^{(t-2)} k)^{k-q+1}}{(\log^{(t-2)} c)^c}
...
\citet{dujmovic.morin.ea:structure} show a similar result for some non-minor-closed families of graphs, the most well-know of which are the $k$-planar graphs:

\begin{thm}[dujmovic.morin.ea:structure]\label{gk-planar}
