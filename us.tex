...
\begin{lem}[\cite{dujmovic.morin.ea:notes}]\label{simple-subgraph}
    For any graph $G$ with $\stw(G)\le t$, there exists a simple $t$-tree $H$ with $V(G)= V(H)$ and $E(G)\subseteq E(H)$.
\end{lem}


\begin{lem}[\cite{dujmovic.morin.ea:notes}]\label{simple-bfs-layers}
...
\begin{proof}
    ...
    \[
        n_{X}:=\sum_{v\in V(H')}\frac{(\log^{(t-2)} k)^k}{(\log^{(t-2)}\gamma(v))^{\gamma(v)}} \enspace .
    \]
...
We are now ready to prove the technical statement that implies \weirdref{simple-t-trees}{a}. Note that in the following statement $\stw(H)\le t$, not $t-1$.  This is the whole enchilada:


\begin{lem}\label{two-tree-technical}
    For every fixed positive integer $t$ there exist an integer constant $a>0$ such that, for every integer $k\ge \tau(t)$, every $n\le (\log^{(t-2)} k)^{k}$, and every $n$-vertex graph $H$ with $\stw(H)\le t$, $\trn(H)\le ak$.
\end{lem}
...
\begin{proof}
    ...
    Let $\mathcal{T}:=(B_x:x\in V(T))$ be a $t$-simple tree decomposition of $H$ rooted at $r\in V(T)$.  Without loss of generality, we may assume that $H$ is \emph{edge-maximal} with respect to $\mathcal{T}$ so that $vw\in E(H)$ if and only there exists $x\in V(T)$ such that $\{v,w\}\subseteq B_x$.  Let $L_0,\ldots,L_m$ be a BFS layering of $H$ with $L_0=B_r$.
