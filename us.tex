...
A node $x$ in a tree $T$ is a \emph{branching node} if $x$ has degree at least 3.  Let $\Lambda(T)$ denote the set of branching nodes in a tree $T$.  Let $H$ be a $t$-tree with a rooted canonical tree decomposition $\mathcal{T}:=(B_x:x\in V(T))$.  For each node $x\in V(T)$, define the parent-set $P_x:=\{x_T(v):v\in B_x\}$.  Define the $\mathcal{T}$-skeleton} of $H$ as the graph $H[\bigcup_{x\in\Lambda(T)}\bigcup_{y\in P_x} B_y]$.
...
\begin{proof}
    ...
    \begin{compactenum}
        ...
        \begin{compactenum}
            \item $\{u_1,\ldots,u_r\}\subseteq V(H-V(\hat{H}))$.  In this case, $u_0,\ldots,r_r$ is a path in $H-V(\hat{H})$, so $\varphi(u_0)\neq \varphi(u_r)$ or $\varhpi(u_0)<\varphi(u_i)$ for some $i\in\{1,\ldots,r-1\}$ since $\varphi$ is an $\ell$-ranking of $H-V(\hat{H})$.

            \item $v_i\in V(\hat{H})$ for some $i\in\{1,\ldots,r\}$. In this case, $\varphi(u_0)\le 3t+1 < \varphi(u_i)$.
        \end{compactenum}
        \item $\{u_0,u_r\}\subseteq V(\hat{H})$. We claim that, in this case $u_1,\ldots,u_r\in V(\hat{H})$.  To see this, consider the sequence of nodes $x_0,\ldots,x_r:=x_T(u_0),\ldots,x_T(u_r)$.  
