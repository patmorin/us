...
Now we arrive at the combinatorial core of the proof. The main idea is to cover $H$ with a sequence of blocks, each of which consists of $\ell+1$ consecutive BFS layers.  Each pair of consecutive blocks overlaps in a single BFS layer.  To convey some intuition about the proof, we first sketch how it can be used to prove that any tree $T$ with $3^3\le n \le k^k$ vertices has an $\ell$-vertex ranking $\varphi:V(T)\to\{1,\ldots,ak\}$, for some $a$ that depends only on $\ell$. [There is no requirement that $k$ be an integer here.]  Note that $k=\Theta(\log n/\log\log n)$ so this is already a non-trivial extension of the result by \citet{karpas.neiman.ea:on} that $2$-vertex ranking of $T$ can be done using $O(\log n/\log\log n)$ colours. 
