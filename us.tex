...
\begin{proof}[Proof of \cref{simple-t-trees} (upper bound)]
  ...
  Fix some $\epsilon > 0$, let $k:=(1+\epsilon)\log (tn)/\log^{(t)} n$, and let $c:=\tau(t-2)$, so $\log^{(t-2)} c=0$.
  ...
  \begin{align*}
    ...
    & = (1+\epsilon)\cdot\frac{\log^{(t-2)}\left(\log^{(2)} (tn) + \log(1+\epsilon) - \log^{(t+1)} n\right)}{\log^{(t-2)}\left(\log^{(2)} n\right)} \\
    & \ge (1+\epsilon)\cdot\frac{\log^{(t-2)}\left(\log^{(2)} n + \log(1+\epsilon) - \log^{(t+1)} n\right)}{\log^{(t-2)}\left(\log^{(2)} n\right)} & \text{(since $t\ge 2$)}\\
    & \ge (1+\epsilon)\cdot\frac{\log^{(t-2)}\left(\log^{(2)} n-\log^{(t+1)} n\right)}{\log^{(t-2)}\left(\log^{(2)} n\right)} & \text{(since $\epsilon\ge 0$)} \\
    & \ge (1+\epsilon)\cdot\frac{\log^{(t-2)}\left(\log^{(2)} n-\log^{(3)} n\right)}{\log^{(t-2)}\left(\log^{(2)} n\right)} & \text{(since $t\ge 2$)} \\
