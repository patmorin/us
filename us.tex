...
\todo{Move the alternative definition somewhere}
...
\begin{proof}
    ...
    For each $i\in\{1,\ldots,p\}$, let $P_i':=y_{i-1},\ldots,y_i$, let $G'_i:=G'[\bigcup_{j=y_{i-1}}^{y_i}B'_j]$ and let $\mathcal{P}'_i:=(B'_x:x\in V(P'_i)\}$.
...
\begin{proof}[Proof of \cref{bounded-genus}]
    \todo{Update this to use the new result on bounded genus graphs.}
...
Our upper bounds are constructive and lead to straightforward $O(n)$ time algorithms for finding $\ell$-rankings of (simple) treewidth $t$ graphs, including planar 3-trees, and outerplanar graphs.  For planar graphs, we require an algorithmic version of \cref{product-structure}. Currently, the fastest such algorithm runs in $O(n\log n)$ time \cite{morin:fast}.\todo{Update to reflect linear time algorithm.}
