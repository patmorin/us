...
A path $P:=u_0,\ldots,u_r$ in a graph $H$ is an \emph{induced} path if $H[\{u_0,\ldots,u_r\}]$ is a path.  The following simple lemma shows that, for vertex rankings, we need only consider induced paths:
...
\begin{proof}
    ...
    For the other direction, suppose $H$ contains a not necessarily induced path $u_0,\ldots,u_r$ of length $r\le\ell$ with $\varphi(u_0)=\varphi(u_r)$ and $\varphi(u_0)=\max\{\varphi(u_0),\ldots,\varphi(u_r)\}$.  Then let $w_0,\ldots,w_s$ be the shortest path from $w_0:=u_0$ to $w_s:=u_r$ in  the graph $G[\{u_0,\ldots,u_r\}]$.  Then $w_0,\ldots,w_s$ is an induced path in $G$ with $\varphi(w_0)=\varphi(u_0)=\varphi(u_r)=\varphi(w_s)$. Since $\{w_0,\ldots,w_s\}\subseteq\{u_0,\ldots,u_s\}$, $\varphi(w_0)=\varphi(u_0)\le\max\{u_0,\ldots,u_r\}\le\max\{w_0,\ldots,w_s\}$.
\end{proof}
...
\end{inparaenum}
The \emph{width} of $\mathcal{X}$ is $\max\{|B_x|:x\in V(X)\}-1$. We say that a graph $G$ is \emph{edge-maximal} with respect to $\mathcal{X}$ if, for each $x\in V(X)$, $B_x$ forms a clique in $G$. The $X$-decomposition $\mathcal{X}$ is \emph{normal} if, for each edge $xy\in E(X)$, $|B_x\setminus B_y|\le 1$ and $|B_y\setminus B_x|\le 1$.  For any $X$-decomposition of $G$, it is straightforward, and common, to obtain a normal $X'$-decomposition of $G$ where $X'$ is a graph obtained by subdividing edges of $X$.
...
\begin{proof}
    ...
    To obtain $U_i'$ we apply induction on $G_i$ using a path decomposition on a path whose endpoints are $y_{i-1}$ and $y_i$ having bags $B'_{y_{i-1}}=B_{y_{i-1}}\setminus u_{i-1}$ and $B'_{y_{i}}=B_{y_{i}}\setminus u_{i}$.  Therefore $U$ contains $\{u_{i-1},u_i\}\cup B'_{y_{i-1}}\cup B'_{y_i}=B_{y_{i-}}\cup B_{y_i}$.
    ...
    \end{inparaenum}
    First note that, for any such subpath $\{w_a,w_b\}\subseteq U$, so we need only show that $w_{a+1},\ldots,w_{b-1}$ is contained in $U$.  There are three cases to consider:
...
%     % $\mathcal{T}$-downward path in $H$ of length $q\le\ell$ with $\{w_0,w_q\}\subseteq \bigcup_{j=0}^{i-1} V_{j}$ and $w_1,\ldots,w_{q-1}\in V_{i}\setminus \bigcup_{j=0}^{i-1} V_{j}$.
%
...
\begin{proof}
    ...
    \begin{compactenum}
        ...
        \begin{compactenum}
            \item $\{u_1,\ldots,u_{r-1}\}\subseteq V(H-V(\hat{H}))$.  In this case, $u_0,\ldots,r_r$ is a path in $H-V(\hat{H})$, so  $\varphi(u_0)<\varphi(u_i)$ for some $i\in\{1,\ldots,r-1\}$ since $\varphi$ is an $\ell$-ranking of $H-V(\hat{H})$.
...
Let $t,k,d,\ell,g\in\N$ and $c\in\R$ with $t\ge 2$ and $\tau(t)\le c\le k-1$; let $H$ be a graph with $\diam(H)\le d$ and $\stw(H)\le t$ in which each vertex $v\in V(H)$ is assigned a real-valued weight $n_v\ge 1$. Then there exists an integer constant $a:=a(t,\ell,d,g)$ such that, if
\begin{equation}
     n_H := \sum_{v\in V(H)} n_v \le \frac{(\log^{(t-2)} k)^k}{\left(\log^{(t-2)} c\right)^{c}} \enspace ,
...
\begin{proof}
    ...
    \begin{equation}
        n_0 := \frac{(\log^{(t-2)} k)^k}{\left(\log^{(t-2)}(c+s)\right)^{c+s}} \enspace . \label{heavy-def}
    \end{equation}
    where $s:=(\log c)/\log^{(t-1)} c$.
    % so that $\gamma_{t-2,k}(n_0)= c+s+\log(c+s)/\log^{(t-1)}(c+s)$.
    We say that a subgraph $X$ of $H$ is \emph{heavy} if $n_X>n_0$ and $X$ is \emph{light} otherwise.  For a heavy subgraph $X$,
    \[ \frac{|H|}{n_X} > \frac{|H|}{n_0} \in \Omega\left(\frac{|H|}{c}\right)
    \enspace.
    \]
    \begin{align*}
        (\log^{(t-2)} k)^k/n_X
        ...
        & = O\left(c\cdot\left(\log^{(t-2)}c\right)^{c}\right) \enspace .
        \numberthis \label{sizer}
    \end{align*}
    % since $c+s\in O(c)$.

    By \cref{weighted-separator} with the weight function $\xi(v):=n_v$, there exists $S_T\subseteq V(T)$ of size $O(c)$ that defines $S:=\bigcup_{x\in S_T} B_x$ such that each component $X$ of $H-S$ is light.  Let $T'$ be the subtree of $T$ induced by $S_T$ and every $T$-ancestor of every node in $S_T$, i.e., $T':=T[\bigcup_{x\in S_T} V(P_T(x))]$. Let $H':=H[\bigcup x\in V(T') B_x]$.  Observe that $\mathcal{T'}:=(B_x:x\in V(T'))$ is a $t$-simple tree decomposition of $H'$, so $\stw(H')\le t$.

    For each $i\in\{0,\ldots,h\}$, let $H'_i:=H'[L_i]$.  By \cref{simple-bfs-layers},  $\stw(H'_i)\le t-1$.  Furthermore, $\mathcal{T}'_i:=(B_x\cap L_i: x\in V(T'))$ is a tree decomposition of $H'_i$ and $H'_i$ is edge maximal with respect to $\mathcal{T}'_i$.  For each leaf $x$ of $T'$ is an element of $S_T$.  Therefore the number of leaves of $T'$ is at most $|S_T|\in O(c)$. Since $T'$ has $O(c)$ leaves, it has $O(c)$ branching nodes.  Therefore, by \cref{skeleton-size}, the $(\mathcal{T}_i',\ell)$-skeleton $\hat{H}_i'$ of $H_i'$ has size $|\hat{H_i}'|\in O(c)$.
    ...
    In the preceding paragraphs, we have defined a colouring $\varphi: V(H')\to \{a(k-c-1)-(h+1)r-g,\ldots,a(k-c-1)-g\}$.    For a sufficiently large constant $a:=a(t,\ell,d,g)$, $\lceil a(k-c-1)\rceil-(h+1)q >  a(k-c-s-1)+1$, so each vertex in $H'$ receives a colour larger than $a(k-c-s-1)+1$.  For any vertex $v\in V(H)$,
   ...
   % Finally, we augment $\varphi$ with a lower-order colour that tells us which BFS layer each vertex is on.  Formally, we redefine $\varphi(v):=h\cdot \varphi(v) + d_H(v,L_0)$.

   All that remains is to verify that $\varphi$ is an $\ell$-ranking of $H$. To do this, consider any path $u_0,\ldots,u_r$ in $H$ with $\varphi(u_0)=\varphi(u_r)$.  By \cref{induced-paths-only} we may assume that $u_0,ldots,u_r$ is an induced path in $H$.  We must show that $\varphi(u_0)<\varphi(u_i)$ for some $j\in\{1,\ldots,r-1\}$. There are a few cases to consider:
   \begin{compactenum}
        ...
        \begin{compactenum}
           \item $\{u_0,\ldots,u_{r}\}\subseteq V(H'_i)$ in which case $\varphi(u_j)>\varphi(u_0)$ for some $j\in\{1,\ldots,r-1\}$ since $\varphi$ is an $\ell$-ranking of $H'_i$ (by the application of \weirdref{simple-t-trees}{a}) to $H'_i$; or
           \item $u_j\in V(H'_{i-1})$ for some $j\in\{1,\ldots,r-1\}$.  In this case $\varphi(u_j)>a(k-)\varphi(u_0)$.
       \end{compactenum}
       ...
       \begin{compactenum}
            ...
            \item $X=Y$ and $\{u_0,\ldots,u_r\}\subseteq V(X)$, in which case $\varphi(u_j)>\varphi(u_0)$ for some $j\in\{1,\ldots,r\}$ (by the application of induction on $X$). \qedhere
        \end{compactenum}
    \end{compactenum}
\end{proof}
...
\begin{lem}\label{two-tree-technical}
    Let $n,k,t,\ell\in N$ and $c\in\R$ be such that $n\le (\log^{(t-2)} k)^k/(\log^{(t-2)} (c+s))^(c+s)$ where $s:=\log c/\log^{(t-1)} c$;
...
\begin{proof}
    ...
    Otherwise, consider the subgraph $I:=H[L_{\ell+1}]$.  By \cref{simple-bfs-layers}, $\stw(I)\le t-1$.  We say that a maximal clique $C$ in $I$ is \emph{dangerous} if $|H_C|\ge (\log^{(t-2)} k)^/(\log^{(t-2)} c')^{c'}$, where $c':=c+s+\log(c+s)/\log^{(t-1)}(c+s)$ and $C$ is \emph{safe} otherwise.  The same computation used in the proof of \cref{t-trees-slack} shows that the number of dangerous cliques in $I$ is $O(c)$.

    For each $v\in\bigcup_{j=0}^\ell L_j$, set $n_v:=1$.  For each each $v\in L_{\ell+1}= V(I)$, set $n_v:=1+\sum_{C\in N_{\ell+1}(v)} |H_C|$.  Let $H_{0}:=H[\bigcup_{j=0}^\ell L_j]$  By \cref{size-claim}, $\sum_{v\in V(H_{\le\ell})} n_v \le t\cdot |H|\le \cdot t\cdot (\log^{(t-2)} k)^k/(\log^{(t-2)}(c+s))^{(c+s)}$.  By \cref{t-tree-slack}, $H_{0}$ has an $\ell$-ranking $\varphi:V(H_0)\to\{1,\ldots,\lfloor a(k-c-1)\rfloor\}$ in which $\varphi(v)> a(k-\gamma_v-1)$ for each $v\in V(H_0)$.  We make the following modifications to $\varphi$.

    \begin{compactenum}
        \item We set $\varphi(v_i):=\phi_i$ for each $i\in\{1,\ldots,t\}$.
        \item For each vertex $v$ that takes part in a dangerous clique $C$ of $I$, we set $\varphi(v)$ to a distinct value in $\{\lfloor a(k-c-1)\rfloor+1,\ldots,ak\}\setminus\{\phi_1,\ldots,\phi_t\}$.
    \end{compactenum}
    These modifications ensure that $\varphi$ satisfies requirements (R1) and (R2) and, since it only introduces new unique colours larger than any existing colour, it preserves the fact that $\varphi$ is an $\ell$-ranking of $H_0$.

    Next, we apply induction on $H_C$ for each maximal clique $C$ in $I$.  Let us verify that this is possible.  If $C$ is a dangerous clique, then $\varphi(v)>a(k-c-1)$ for each $v\in C$ and $|H_C|\le |H|\le(\log^{(t-2)}k)^k/(\log^{(t-2)}(c+s))^{c+s}$.  Therefore, the colours assigned to $C$ by $\varphi$ satisfy the requirements on $\phi_1,\ldots,\phi_t$ and we can apply induction on $H_C$.

    % If $C$ is a safe clique, $n_v \ge |H_C|$ for each $v\in C$.  This implies that $\gamma_v \le \gamma_{t-1,k}(|H_C|)$.  Stated another way, $|H_C|= (log^{(t-2)}k)^k/\log^{(t-2)\delta}^\delta$
    %
    ...
    %
    %
    % If $m<\ell$ then, by \cref{t-trees-slack}, with the weight function $w$
    %
    %
    %  then $H=H[L_0]$ is a $t$-clique

\end{proof}

%
%     let $L_0,\ldots,L_m$ be a BFS layering of $H$ with $L_0:=B_r$; and .
% \end{lem}
%
