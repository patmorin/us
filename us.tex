...
The remainder of this paper is organized as follows: \Cref{sec:basics} reviews some basic tools used in the following sections.   \Cref{lower-bounds} proves the lower bound in \cref{t-trees}, which immediately implies the lower bounds in \cref{planar,simple-t-trees}. \Cref{upper-bounds} proves the upper bound in \cref{simple-t-trees}, from which the upper bounds in \cref{planar,t-trees,bounded-genus} follow easily.  \Cref{conclusion} gives a brief summary and conclusions.
...
% For any $X$-decomposition of $G$, it is straightforward, and common, to obtain a normal $X'$-decomposition of $G$ where $X'$ is a graph obtained by subdividing edges of $X$.
