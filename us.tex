...
\begin{proof}
    ...
    First we transform $\mathcal{T}$ so that, for each edge $xy\in E(T)$ from a parent $x$ to its child $y$, $|B_y\setminus B_x| \ge 1$.  This is easy, if $B_y\subseteq B_x$ then we can contract $y$ into $x$ without affecting $t$-simplicity.

    % When no such operation is possible, $|B_y\setminus B_x|\ge 1$ for every edge $xy\in E(T)$.

    % If every node $x$ of $T$ has $|B_x|= t+1$, then we are done; $\mathcal{T}$ is a canonical tree decomposition of a simple $t$-tree $H$.  Otherwise,
    % there exists some non-leaf node $x$ with $|B_x|<t+1$.
    %
    %
    %
    % such that $|B_a|=t+1$ for every strict $T$-ancestor $a$ of $x$.
    %
    % Next we want to transform $\mathcal{T}$ so that the bag, $B_r$ at the root $r$ of $T$ has size exactly $t+1$.  Suppose that $B_r$ does not

    Now, we say that $\mathcal{T}$ is \emph{maximally deep} if no node $x$ has two children $y$ and $z$ with $B_y\cap B_x\subseteq B_z\cap B_x$.  If such a configuration of nodes exists, then we can remove the edge $xy$ and add the edge $zy$, to make $y$ a child of $z$.  This does not affect $t$-simplicity and increases the total path length of $T$, which is upper bounded by $(|T|^2+|T|)/2$.  Therefore after a finite number of such operations, $\mathcal{T}$ is maximally deep.

    We say that $\mathcal{T}$ is \emph{uniform} if there is no node $x$ with child $y$ such that $|B_y\setminus B_x|>1$.  Suppose $\mathcal{T}$ is not uniform and that $B_y\setminus B_x=v_1,\ldots,v_d$ for some parent-child edge $xy\in E(T)$.

    If $|B_y|\le t$, then we can replace the edge $xy$ with a path $x,x_1,\ldots,x_{d-1},y$ where, for each $i\in\{1,\ldots,d\}$, $B_{x_i}=\{v_1,\ldots,v_i\}\cup (B_x\cap B_y)$.  We use the convention that $x_0:=x$ and $x_d:=y$. This does not affect $t$-simplicity since each newly introduced node $x_i$ has $|B_{x_i}|<|B_y|\le t$.

    If $|B_y|=t+1$, then performing the same operation may affect $t$-simplicity. In particular, the newly introduced bag $C:=B_{x_{d-1}}$ has size $t$ and $C\subset B_y$.  There is at most one child $z$ of $y$ with $C\subset B_z$.  If no such child $z$ exists, then the resulting tree decomposition is $t$-simple.  If, however the child $z$ does exist, then we perform the following operation:

    We make two children $y'$ and $z'$ of $x_{d-2}$ whose bags contain $t$-element subsets of $B_x$ that each contain $B_x\cap B_y$.   Note that there is still a danger that this violates $t$-simplicity if $|B_x|=t-1$ and one of these nodes contains a copy the intersection between $B_x$ and $B_x$'x parent bag. [I guess in general, we'll make that happen.]   If this happens, then the maximal deepness property (at $x$) ensures that $B_x$ contains a useless vertex and we can replace one of our new nodes with $y$ or $z$.


     contains  $\{v_1,\ldots,v_{d-1}\}\cup (B_x\cap B_y)$ and $\{v_1,\ldots,v_{d-2}\} whose children are $y$ and $z$, respectively.



    We make $y$ and $z$ children of $x_{d-1}$.


    and there is no child $z$ of $y$ such that $B_z\supseteq\{v_1,\ldots,v_{d-1}\}\cup (B_x\cap B_y)$, then we perform the same operation, replacing $xy$ with $x,x_1,\ldots,x_{d-1},y$.  Again, this does not affect $t$-simplicity since $|B_{x_i}|<t$ for each $i\in\{1,\ldots,d-2\}$ and the the $t$-element subset $B_{x_{d-1}}$ appears is contained only in $B_{x_{d-1}}$ and $B_y$.

    Finally, if $|B_y|=t+1$ and $B_{}

    , then there is at most one child $z$ of $y$ that contains $x_1,\ldots,x_{d-1}.


    Next we would like to tranform $\mathcal{T}$ so that, for every parent-child edge $xy\in E(T)$, $|B_y\setminus B_x|=1$.


    Next, consider some edge $xy\in E(T)$ joining node $x$ to its child $y$.  Suppose $B_y\setminus B_x = v_1,\ldots,v_d$ for $d>1$.   we can replace the edge $xy$ with
    edges $xx_1$, $x_1y$ and $x_1


     a path $x,x_1,\ldots,x_{d-1},y$ in which $B_{x_i}$ contains $(B_x\cap B_y)\cup\{v_1,\ldots,v_i\}$.   The only possible violation of $t$-simplicity here comes from the node $v_{d-1}$, since $B_{v_{d-1}}$ is the only newly introduced node whose bag has size at least $t$.  The danger is that $B_{v_{d-1}}\subseteq B_z$ for some node $z\neq y$.  This could only occur if $B_z\supseteq B_x\cap B_y$.  Since $\mathcal{T}$ is maximally deep, any such $z$ must be a strict $T$-descendant of $y$.  However, ..........

     is not a child of (or descendant of) $x$.  The only possibility is that $z$ is a strict $T$-descendant of $y$.    Otherwise, we can replace $



    [TODO]
    % Let $\mathcal{T}:=(B_x:x\in V(T))$ be a $t$-simple tree decomposition of $G$.  By a standard argument we may assume there are no two distinct nodes $x,y\in V(T)$ such that $B_x\subseteq B_y$.  We may further assume that, for each node $x\in V(T)$, the vertices of $B_x$ form a clique in $G$.
    %
    % The proof is by induction on $|T|$.  If $|T|=1$, so that $T$ has only one node $r$, then $G$ is contained in a clique of size $|G|$.
    %
    % If $|T|>1$, $|B_x|=t+1$ for each $x\in V(T)$, and $|B_x\cap B_y|=t$ for each edge $xy\in E(T)$, then $G$ is a simple $t$-tree and there is nothing to prove.
    % \begin{enumerate}
    %     \item If there exists a non-leaf node $x\in V(T)$ such that $|B_x|<t+1$, then consider two graphs formed by $G-B_x$ and $G-x
    %
    % If there exists a leaf $x\in V(T)$ such that $|B_x|<t+1$ then take vertices from $B_y$ where $y$ is the parent of $x$.
    %
    %
    % $B_x$  graph $G-B_x$.
    %
    %  remove   Otherwise, there exists an edge $xy\in E(T)$ such that $|B_x\cap B_y|<t$.
    %
    %
...
\begin{proof}
   ...
   \begin{align*}
        ...
        & \le \left((\log^{(t-2)} h)^{(t-1)h}\cdot m\cdot h\right)^{m} \\
        ...
        & \le (\log^{(t-1)} r)^{tr} & \text{(since $r\ge \tau(t)$, so $\log^{(t-1)} r\ge e$).} & \qedhere
   \end{align*}
\end{proof}

\begin{proof}[Proof of \cref{t-trees} (lower bound)]
    \Cref{treewidth-lower-bound} produces a graph $G$ of size $n \le (\log^{(t-1)} r)^{tr}$ and $\trn(G)\ge r$.  So,
    \[  \log n \le tr\log^{(t)} r = t\cdot\trn(G)\log^{(t)} \trn(G)\]
