...
In order to prove eventually prove a tight bound for planar 3-tree (\cref{three-trees}) we introduce a light abstraction.  We say 
...
Reviewing the proof of \cref{t-trees-tehcnical}, which relies on \cref{t-trees-no-slack,t-trees-slack} we see that it can be strengthened as follows:

\begin{lem}
    Let $r,t$ be fixed, let $H$ be an $n$-vertex $t$-tree, let $v_1,\ldots,v_n$ be a construction order for $H$ and let $L_0,\ldots,L_m$ be a BFS layering of $H$ with $L_0:=\{v_1,\ldots,v_{t+1}\}$.  If $\uqs(H[S]) \in O(\log |S|/\log^{(r)} |S|)$, for each $S\subseteq L_i$ and each $i\in\{1,\ldots,m\}$, then $\uqs(H) \in O(\log n/log^{(r)}n)$.
\end{lem}
...
To complete the upper bound in \cref{planar},  all that remains is to prove the upper upper in \cref{three-trees} on the number of colours required by 2-rankings of $n$-vertex planar 3-trees.  This proof is essentially a sharpening of the arguments in the previous section using the additional property of planarity.  Specifically, while a BFS layer in a layering of a 3-tree is a 2-forest, a BFS layer of a planar 3-tree is an outerplanar graph.  The following theorem shows that, with respect to $2$-ranking, outerplanar graphs behave more like trees than 2-trees:
