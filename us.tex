...
\begin{proof}
    ...
    % (Alternatively, $L_0,\ldots,L_{m}$ are the breadth-first-search layers of $U^{(h,m)}$ rooted at $a_0$.)
...
% Asymptotic notations $O(\cdot)$, $\Omega(\cdot)$, and $o(\cdot)$ are used assuming that $n$ (or some increasing function of $n$) is the only parameter tending to infinity.
...
\begin{compactenum}
    ...
    % and $O(1)$ small colours in $\{a(k-c-2)+1,\ldots,a(k-c-1)\}$ (that may be used repeatedly); and
...
% The separator $S$ has a \emph{shadow-completeness} property: if $uvw$ is a path with $u,w\in S$ and $v\in V(H'-S)$, then $uw\in E(H'[S])$.  This ensures that the resulting colouring is a 2-ranking of $H'$.
...
Our proof is based on the following claim:
    ...
    \begin{proof}[Proof of \cref{main-claim}]
        ...
        % In this case, for each $v\in V(H'[L_i])$, we set $\gamma(v)$ equal to the solution of $(\log^{(t-2)} k)^k/(\log^{(t-2)} \gamma(v))^{\gamma(v)} = n_v$.
        Now,
        \[
