...
\section{Lower Bound for $t$-Trees}
...
\begin{thm}\label{treewidth-lower-bound}
    For each integer $t\ge 1$, there exists $\alpha,\beta>0$
...
\begin{proof}
    ...
    % The first base case, $t=0$, is trivial; For every $n\ge 1$, taking $G$ to be an $\ell^\ell$-vertex graph with no edges gives a graph with $\tw(G)=0$ and
    % \[
    ...
    The base case $t=1$ has already been established by \citet{karpas.neiman.ea:on} who show that the complete $(\ell+1)$-ary tree $T$ of height $\ell-1$ has $\uqs(T)\ge \ell$.  The tree $T$ has size $\sum_{i=0}^{\ell-1} (\ell+1)^i \le \ell^\ell$.
    Observe that
    \[
    ...
    We do this with the value $m:=\log (\ell\log\ell)/\log^{(t)}(\ell\log\ell)$.  Now we take the graph $G:=U^{(h,\ell)}$.  By \cref{boost},
    \begin{align*}
   ...
   \begin{align*}
        |G| & \le \gamma\cdot (\log^{(t-2)}m)^{\alpha' m}\ell h)^\ell \\
...
Since every graph of treewidth at most 2 is planar, we obtain the following corollary from \cref{treewidth-lower-bound}:

\begin{cor}\label{planar-lower-bound}
    There exists a constant $c>0$ such that, for infinitely many $n\in\N$, there is an $n$-vertex planar graph $G$ with $\uqs(G)\ge c\log n/\log^{(3)} n$.
\end{cor}
