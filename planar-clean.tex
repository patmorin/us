...
In this section, the statements of lemmas are not self-contained. They all include the definitions in this paragraph. See \cref{y}.
Let $G$ be an $n$-vertex planar 3-tree with $n\ge 3$;
let $A:=\{v_1,v_2,v_3\}$ be the vertices of a 3-clique in $G$ that is contained in at most one $4$-clique of $G$;
for each $3$-clique $K$ in $G-A$, let $G_K$ be the subgraph of $G$ induced by $K$ and the component (if any) of $G-K$ that does not contain $A$;
let $B:=N_G(A)$;
let $Y:=\{w\in B: |N_G(w)\cap A|\ge 2\}$;
for each $i\in\{1,2,3\}$, let $B_i:=B\cap N_G(v_i)$;
let $V_i:=\{v_i\}\cup\bigcup_{K\in G[B_i]} V(G_K)$ where $K$ ranges over all 3-cliques in $G[B]$ that are incident to $v_i$;\footnote{A 3-clique $k$ is incident to $v_i$ if and only if $G$ contains an edge $wv_i$ with $w\in K$.} and let $\gamma(v_i):=\gamma_1(|V_i|)$.

\begin{figure}[htbp]
    \centering{
        \begin{tabular}{c@{\hspace{1cm}}c}
        \includegraphics{figs/y-1} &
        \includegraphics{figs/y-2}
        \end{tabular}
    }
...
Note, as illustrated on the right hand side of \cref{y},that $\{V_1,V_2,v_3\}$ is not a partition of $V(G)$ and this is the source of extra work.  Even in a planar drawing of $G$ with $A$ on the outer face, the vertices in $V_i$ are not all drawn in the inner faces of $G[B_i]$.


\begin{lem}[Shadow Completeness \cite{kundgen.pelsmajer:nonrepetitive}]
    \label{shadow-completeness}
    Let $u_0,\ldots,u_r$ be a path in $G$ such that $d_G(A,u_0)=d_G(A,u_r)$ and such that $d_G(A,u_i)>d_G(A,u_0)$ for each $i\in\{1,\ldots,r-1\}$.  Then $u_0u_1\in E(G)$.
\end{lem}



Let $C:=N_G(B)\setminus A$.  For each $w\in B$, define $V_w:=\{w\}\cup \bigcup_{K} V(G_K)$, where $K$ ranges over all 3-cliques in $G[C]$ that are incident to $w$.

\begin{clm}\label{size-claim}
    For each $i\in\{1,\ldots,3\}$, $|V_i|\le \sum_{w\in B_i}|V_w| \le 3|V_i|-3$.
\end{clm}

\begin{proof}[Proof of \cref{size-claim}]
    The vertex $v_i$ it not counted in the sum.
    Each vertex $w\in B_i$ is counted exactly once by the sum, as part of $|V_w|$. Each vertex $x\in V_i\cap C$ is incident to at most 3 vertices in $B_i$, so appears in $V_w$ for at most three vertices $w\in B_i$.   Each vertex $y\in V_i\setminus B\setminus C$ appears in $G_{K_y}$ for exactly one 3-clique $K_y$ of $G[C]$. Each $3$-clique of $G[C]$ is incident to at most 3 vertices of $B_i$, so $y$ appears in $V_w$ for at most three vertices $w\in B_i$. Therefore $n_i=\sum_{w\in B_i}|V_w|\le |B_i| + 3|C\cap V_i| + 3|V_i\setminus C\setminus B_i|\le 3|V_i|-3$. This completes the proof of \cref{size-claim}
\end{proof}


\begin{lem}
    There exists an integer $a$ (not depending on $n$) such that, if all the following preconditions hold:
    \begin{compactenum}[(P1)]
        \item $c_1,c_2,c_3\in\R$ and $c_1,c_2,c_3> e$;
        % \item $k\in\N$ and $n\le (\log k)^k/(\log c)^{c}$;
        \item for each $i\in\{1,2,3\}$, $S_i \subseteq B_i\setminus Y$ and, for each component $I$ of $G[B_i]-Y-S_i$, $\sum_{w\in V(I)} |V_w| \le (\log k)^k/(\log(c_i+s_i))^{c_i+s_i}$, where $s_i:=\log c_i/\log\log c_i$; and
        \item $\varphi:A\cup S_1\cup S_2\cup S_3\to\N$ has the following properties:
        \begin{compactenum}
            \item\label{distinct-outer} $|\{\varphi(v_i): i\in\{1,2,3\}|=3$;
            \item\label{distinct-separator} for each $i\in\{1,2,3\}$, $|\{\varphi(w): w\in S_i\}|=|S_i|$; and
            \item\label{input-range}for each $i\in\{1,2,3\}$ and each $w\in \{v_i\}\cup S_i$, $a(k-\gamma(v_i)-1)-3< \varphi(w)\le ak$
        \end{compactenum}
    \end{compactenum}
    % , where $s:=\log c/\log\log c$,
    then there exists $\varphi:V(G-A-S_1-S_2-S_3)\to\N$, $\psi:V(G-A)\to\{1,\ldots,3\}$,  and $\eta:V(G-A-B)\to\{1,\ldots,3\}$  such that
    \begin{compactenum}[(R1)]
        % \item $\varphi$ is a proper colouring of $G$; \label{proper}
        \item $1\le\varphi(v)\le ak$ for each $v\in V(G)$; \label{range}
        \item\label{ranking} for each $i\in\{1,2,3\}$ and each induced path $uvw$ in $G$ with $u,w\subseteq V_i$ and $d_G(u,A)=d_G(w,A)$,
        \begin{compactenum}[(i)]
            \item $\varphi(u)\neq\varphi(w)$, $\psi(u)\neq\psi(w)$, $\eta(u)\neq\eta(w)$, or
            \item $\varphi(u)<\varphi(v)$.
        \end{compactenum}
    \end{compactenum}
\end{lem}

\begin{proof}
    The proof is by induction on $|G|$.  The base case $|G|=3$ is trivial. The function $\varphi$ is completely defined by the statement of the lemma and the fact that $\varphi(v_1),\varphi(v_2),\varphi(v_3)$ (P\ref{distinct-outer}) are distinct ensures that $\varphi$ satisfies (R\ref{proper}) and (R\ref{ranking}).  The requirement (R\ref{range}) follows from (P\ref{input-range}).  The function $\psi$ is irrelevant.  Now assume $|G|\ge 4$.
    ...
    % The idea behind the colouring is the following.  Each vertex $w$ in $B$ is incident to a subset $A_w\subset A$ of $\{v_1,v_2,v_3\}$.  In most cases, $w$ will receive a colour $\varphi(w)\le \lfloor a(k-\max\{\gamma(v):v\in A_v\}-1)\rfloor$. In this way most vertices in BFS level $i+1$ (vertices in $B$) receive a colour smaller than any of their parents in BFS level $i$ (vertices in $A$).

    % The strategy for assigning colours differs depending on whether $w$ has one or two neighbors in $A$.

    Each vertex in $B$ is incident to a non-empty subset $A_w\subset A$ and we let $\phi_w:=\lfloor a(k-\max\{\gamma(v):v\in A_w\}-1)\rfloor$, so that $\phi_w$ is smaller than $\varphi(v)$ for each parent $v$ of $w$ in $A$.  Most vertices $w\in B$ will be coloured with a colour smaller than $\phi_w$.

    The graph $G[Y]$ is a tree with at most 3 leaves rooted at the unique node $r\in B_1\cap B_2\cap B_3$ and root-to-leaf paths $P_{i,j}$ corresponding to $B_i\cap B_j$ for each distinct pair $i,j\in\{1,2,3\}$.  Without loss of generality, assume $\gamma(v_1)\ge \gamma(v_2)\ge \gamma(v_3)$.  We first colour the paths $P_{1,2}$ and $P_{3,1}$ using the three colours $\{\phi_r,\phi_{r-1},\phi_{r-2}\}$.  Next we colour the path $P_{2,3}$ using the colours $\{\phi,\phi-1,\phi-2\}$ where $\phi=\lfloor a(k-\gamma(v_2)-1)\rfloor$.

    Now that we have coloured $Y$, we move on to colouring $G-A-Y$. Let $S:=S_1\cup S_2\cup S_3$.  Consider some component $I$ of $G[B]-S-Y$. The vertices in $I$ are all incident to exactly one vertex $v_i\in A$. The component $I$ is contained in some component of $G[B_i]-S_i$ so, by (P3), $\sum_{w\in V(I)}|V_w|\le (\log k)^k/(\log(c_i+s_i))^{c_i+s_i}$.    Therefore, the graph $I$ with the weight function $\gamma$ satisfies the requirements for \cref{outerplanar-lemma}. Applying \cref{outerplanar-lemma} with $g=3$ gives a 2-ranking $\varphi:V(I)\to \{1,\ldots,a(k-c_i-1)-3\}$ of $I$ such that $\varphi(w)> a(k-\gamma(w)-1)$ for each $w\in V(I)$.

    Thus far, we have defined $\gamma$ for all $A\cup B$ and now we extend it to the rest of $V(G)$ by applying induction on each subgraph $G_K$ attached to each $3$-clique $K$ in $G[B]$.  This requires that, for each $3$-clique $K=\{w_1,w_2,w_3\}$ in $G[B]$, we define three separators $S_{w_1}$, $S_{w_2}$ and $S_{w_3}$ that satisfy (P1)--(P4).

    Define $D:=N_G(C)\setminus B$. For each $x\in C$, define $V_x:=\{x\}\cup \bigcup_{K} V(G_K)$, where $K$ ranges over all 3-cliques in $G[D]$ that are incident to $x$.  Observe that, if $x\in V_w$ for some $w\in B$, then $V_x\subseteq V_w$.  Therefore, by \cref{size-claim}, $\sum_{x\in V_w\cap C}|V_x|\le 3|V_w|$.  Let $\gamma(w):=\gamma_1(|V_w|)$.

    For each $w\in B$, $\tw(G[V_w\cap C])\le 2$.  Therefore, for any $d_w\ge 1$, there exists a \emph{separator} $S_w'\subseteq V_w\cap C$, $|S_w'|\le 2d_w$ such that, for each component $I$ of $G[V_w\cap C]-S_w'$, $\sum_{x\in V(I)} |V_x| \le \sum_{x\in V_w\cap C}|V_x|/d_w \le 3|V_w|/d_w$. A boring calculation shows that, for some $d_w\in O(\gamma(w))$,
    \begin{equation}
        \frac{3|V_w|}{d_w} = \frac{3(\log k)^k}{d_w(\log \gamma(w)^{\gamma(w)}} \le
        \frac{(\log k)^k}{(\log (\gamma(w)+s_w))^{\gamma(w))+s_w}}
        \label{s_w-size}
    \end{equation}
    where $s_w:=\log \gamma(w)/\log\log \gamma(w)$.

    Let $Y_C:=\{x\in C: |N_G(x)\cap B|\ge 2\}$. For each $w\in B$, let $S_w=S_w'\setminus Y_C$.  Observe that, for distinct $u,w\in B$, the sets $S'_u$ and $S'_w$ may overlap, but $S_u\cap S_{w}=\emptyset$.  For each $w\in B$, we let $\varphi:S_w\to \{\lfloor a(k-\gamma(w)-1)\rfloor+1,\ldots,ak\}$ be any injective function so that each element of $S_w$ gets a distinct colour.  For a sufficiently large constant $a$, this is always possible since $|S_w|\le 2d_w\in O(\gamma(w))$.

    Finally, we are ready to apply induction.  For each 3-clique $K:=\{w_1,w_2,w_3\}$ in $B$, define $S_{K,i}:= S_{w_i}\cap V(G_K)$ for each $i\in\{1,2,3\}$ and $c_{K,i}:=\gamma(w_i)$.  We claim that $K$, $G_K$, $c_{K,1},c_{K,2},c_{K,3}$, and $S_{K,1},S_{K,2},S_{K,3}$ satisfy the requirements (P1)--(P3) for the sets $A$, $G$, $c_1,c_2,c_3$ and $S_1,S_2,S_3$, respectively.  That these satisfy (P1) is obvious. That these satisfy (P2) follows from the definition of $S_w$ for each $w\in K$ (see \cref{s_w-size}).  That these satisfy (P3a) comes fromt the fact that $\varphi$ is a proper colouring of $G[B]$ [TODO: Justify]. That these satisfy (P3b) comes from the definition of $\varphi$ over $S_w$ as a injective function.    The requirement (P3c) actually has two parts.  The first part requires that, for each $i\in\{1,2,3\}$ and each $x\in S_w$, $\varphi(x)>a(k-\gamma(w_i)-1)$ and is true by the definition of $\varphi$ over the set $S_w$.  The second part requires that, for each $i\in\{1,2,3\}$, $\varphi(w_i)>a(k-\gamma(w_i)-1)$.  That this second part is satisified can be verified by considering the three ways in which $w_i$ might have received its colour:

    \begin{compactenum}[(i)]
        \item If $w_i\in S_j$ for some $j\in\{1,2,3\}$ then, by (P3c) $\varphi(w_i)>a(k-\gamma(v_j)-1)-3\ge a(k-\gamma(w_i)-1)-3$ since $|V_j|>|V_{w_i}|$.

        \item If $w_i\in Y$ then $\varphi(w_i)>\lfloor a(k-\max\{v\in A_w:\gamma(v)\}-1)\rfloor-2 \ge a(k-\gamma(w_i)-1)-3$.

        \item Otherwise $w_i\in B\setminus S\setminus Y$, so $w_i$ received its colour $\varphi(w_i)$ through an application of \cref{outerplanar-lemma}, which guarantees that $\varphi(w_i)>a(k-\gamma(w_i)-1)$.
    \end{compactenum}

    Therefore, apply induction on $G_K$ yields colourings $\varphi$ and $\psi$ that satisfy (R1)--(R3).  This completes the description of $\varphi$ over all vertices of $G$, completes the description of $\psi$ over all vertices of $G-A-B$, and completes the description of $\eta$ over all vertices of $G-A-B-C$.  To complete the description of $\psi$, for $w\in B$, we set $\psi(w)$ to one of $\{1,2,3\}$ indicating that $w$ is adjacent to $v_{\psi(w)}$. In case $w\in Y$.  To complete the description of $\eta$, for each $i\in\{1,2,3\}$, each 3-clique $K$ in $G[B_i]$ and each vertex $x\in V(G_K[C])$, we set $\eta(x):=i$.

    All that remains is to prove that $\varphi$, $\psi$ and $\eta$ satisfy (R1) and (R2).  Condition (R1) is obvious from definitions. For Condition (R2) there are several cases to consider.
    \begin{enumerate}
        \item $\{u,v,w\}=A$. This case is ruled out by the assumption that $uvw$ is an induced path.
        \item $\{u,w\}\subset A$, $v\in B$; or $\{u,w\}\subset B$, $v\in C$.  These cases are ruled out by \cref{shadow-completeness} and the assumption that $uvw$ is an induced path.
        \item $\{u,v,w\}\subseteq B$. There are three subcases to consider:
        \begin{compactenum}
            \item $\{u,w\}\in Y$. (then $uw\in E(G[Y])$ by shadow completeness so $\varphi(u)\neq\varphi(w)$)
            \item $\{u,w\}\in S$. (then $v\in Y$ or $\varphi(u)\neq\varphi(w)$)
            \item $\{u,w\}\in B-\setminus S\setminus Y$. (then $v\in S\cup Y$ or $\{u,v,w\}\in V(I)$ for some component $I$ of $G[B]-A-Y-S$.)
        \end{compactenum}
        \item $\{u,w\}\subseteq B$, $v\in A$:
        \begin{compactenum}
            \item $\{u,w\}\in Y$. (then $\varphi(u)<\varphi(v)$)
            \item $\{u,w\}\in S$. (then $v=v_i$ and $\{u,w\}\subseteq S_i$ for some $i\in\{1,2,3\}$ so $\varphi(u)\neq \varphi(w)$)
            \item $\{u,w\}\in B-\setminus S\setminus Y$. (then $\varphi(u)<\varphi(v)$)
        \end{compactenum}
        \item $\{u,w\}\subseteq C$, $v\in B$: This is identical to the preceding case with one exception.  If $u\in S_x$ and $w\in S_y$ for some distinct $x,y\in B$. Then $v\in Y_C$ and $\eta(u)\neq\eta(w)$.
